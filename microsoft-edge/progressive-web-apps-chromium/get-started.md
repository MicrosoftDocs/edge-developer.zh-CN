...
---
# <a name="get-started-with-progressive-web-apps-chromium"></a>渐进式 Web 应用和 chromium (入门)   

渐进式 Web 应用 \ (PWA\) 是逐渐增强 [的 Web 应用][WikiProgressiveEnhancement]。  渐进式增强功能包括类似应用的功能，如安装、脱机支持和推送通知。  还可以打包适用于应用商店的 PWA。  可能的应用商店包括 Microsoft Store、Google Play、Mac 应用商店等。  Microsoft Store 是内置于 Windows 10 的商业应用商店。  

以下指南通过创建简单的 Web 应用并扩展为 PWA，概述了 PWA 基础知识。  已完成的项目适用于新式浏览器。  
...
若要创建空 Web 应用，请按照节点 [Express 应用生成器][ExpressjsApplicationGenerator]中的步骤操作，并命名你的应用 `MySamplePwa` 。  
...
这些命令将创建一个空的 Web 应用并安装任何依赖项。  

现在，您具有一个简单的功能性 Web 应用。  若要启动 Web 应用，请运行以下命令。  
...
现在，你已拥有一个简单的 Web 应用，通过添加 PWA 的三个要求将其扩展为 PWA<!--[3 requirements for PWAs][PwaEdgehtmlIndexRequirements]-->[：HTTPS、Web](#step-1---use-https)[应用清单](#step-2---create-a-web-app-manifest)和服务[工作线程](#step-3---add-a-service-worker)。  
...
[将 Web 应用发布为实时网站][VisualStudioNodejsTutorialPublishAzureAppService]，但请确保服务器已针对 HTTPS 进行配置。  例如，你可以创建 Azure [免费帐户][AzureCreateFreeAccount]。  在 [Microsoft Azure 应用服务上][AzureWebApps] 托管网站，默认情况下通过 HTTPS 提供服务。  
...
若要将应用程序清单添加到 Web 应用，请运行以下操作：  

1.  在Visual Studio中，选择 **"文件**打开  >  **文件夹**"并选择 `MySamplePwa` 之前创建的目录。  
1.  选择 `Ctrl` + `N` 以创建新文件，并粘贴到以下代码段中。  
...
1.  添加名为 .的 512x512 应用图标 `icon512.png` 图像 `/MySamplePwa/public/images` 。  您可以使用示例 [图像][ImagePwa] 进行测试。  
1.  在Visual Studio中 `/public/index.html` ，打开并添加标记内的以下 `<head>` 代码段。  
...
服务工作者是 PWA 背后的关键技术，支持脱机支持、高级缓存和运行之前仅限于本机应用的后台任务等方案。  
...
若要在项目中生成服务工作者，请使用[PWA Builder][PwaBuilderServiceWorker]中的**缓存第一个**网络服务工作者方法。  
...
Web 应用现在具有使用缓存第一策略的服务工作者。  新服务工作者首先从缓存提取资源，然后仅根据需要从网络获取资源。  缓存的资源包括图像、JavaScript、CSS 和 HTML。
...
1.  在 Microsoft Edge 中， `F12` 选择打开 Microsoft Edge DevTools。  选择 **"应用程序**"， **然后选择"服务** 工作人员"以查看服务工作者。  如果未显示服务工作线程，则刷新页面。  
    ...
    :::image type="complex" source="./media/devtools-sw-overview.png" alt-text="Microsoft Edge DevTools Service Worker 概述" lightbox="./media/devtools-sw-overview.png":::
       Microsoft Edge DevTools Service Worker 概述
    :::image-end:::
    
1.  通过展开缓存存储并选择**pwabuilder-precache 查看服务工作器缓存**。 ****  应显示服务工作者缓存的所有资源。  服务工作者缓存的资源包括应用图标、应用程序清单、CSS 和 JavaScript 文件。  
...
1.  尝试将 PWA 作为脱机应用。  在 Microsoft Edge DevTools \ (`F12` \) 中，选择 **"** 网络"，然后将**联机**状态更改为 **"脱机"。**  
...
与服务工作人员一样，推送通知 API 也是基于标准的 API。  推送通知 API 跨浏览器工作，因此代码应在支持 PWA 的任何地方运行。  有关将推送消息发送到服务器上不同浏览器的信息，请导航到["Web 推送"。][NPMWebPush]  

以下步骤改编自 Mozilla 提供的"服务工作者指南[][ServiceWorkerCookbookPushRichDemo]中的推送丰富演示"，该演示具有大量其他有用的 Web 推送和服务工作者方法。  

### <a name="step-1---generate-vapid-keys"></a>步骤 1 - 生成 VAPID 密钥  

推送通知需要 VAPID \ (自愿应用程序服务器标识\) 密钥才能向 PWA 客户端发送推送消息。  联机 \ (提供了多个 VAPID 密钥生成器，例如 [，vapidkeys.com][VapidkeysMain]\) 。  生成后，应获取包含公钥和私钥的 JSON 对象。  保存密钥，以执行以下教程中的稍后步骤。  有关 VAPID 和 WebPush 的信息，请导航到使用 Mozilla 推送服务发送 VAPID 标识的 [WebPush 通知][MozillaServicesSendingVapidWebPushNotificationsPush]。  
...
在新建推送订阅之前，Microsoft Edge 会验证用户是否授予了 PWA 接收通知的权限。  如果没有，浏览器会提示用户授予权限。  如果权限被拒绝，则会引发 `registration.pushManager.subscribe` `DOMException` 一个必须处理的请求。  有关权限管理更多信息，请导航到 [Microsoft Edge 中的推送通知][WindowsBlogsWebNotificationsEdge]。  
...
1.  导航到 `http://localhost:3000` PWA。  当服务工作者激活并尝试订阅 PWA 推送通知时，Microsoft Edge 会提示你允许 PWA 显示通知。  选择 **"允许"。**  
...
1.  模拟服务器端推送通知。  在浏览器中打开 PWA 后，选择 `http://localhost:3000` `F12` 打开 DevTools。  选择 **"**  >  **应用程序服务工作**  >  **线程推送**"将测试推送通知发送到 PWA。  
    ...
    :::row:::
       ...
       :::column-end:::
       :::column span="":::
          如果未选择 \ (或激活\) toast 通知，系统会在几秒钟后自动将其关闭，并排队到 Windows 操作中心。  
...
*   [Web.dev 上的渐进式 Web 应用][WebDevProgressiveWebApps]  
*   [作为渐进式 Web][HackerNewsProgressiveWebApps] 应用的黑客新闻阅读器 - 比较实现示例 \ (黑客新闻阅读器\) PWA 的不同框架和性能模式。  
*   [破坏 PBA][Davrous20191018MythBustingPwasNewEdgeEdition]  
*   [渐进式 Web 应用的渐进路线图][CloudfourThinksProgressiveRoadmapYourWebApp]  
*   [使用渐进式 Web 应用的脱机 POST][MediumWebEdgeOfflinePostsProgressiveWebApps]  
*   [PWA 问答&A][AaronGustafsonNotebookPwaQa]  
*   [Web 上的支持][JoretegBlogBettingWeb]  
...
*   [设计和构建不带框架的渐进式 Web (第 2 部分) ][Smashingmagazine201907ProgressiveWebApplicationFrameworkPart2]  
*   [设计和构建不带框架的渐进式 Web 应用程序 (第 3 部分) ][Smashingmagazine201907ProgressiveWebApplicationFrameworkPart3]  
