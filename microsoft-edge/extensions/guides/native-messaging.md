...
通过 Windows 10 创意者更新，Microsoft Edge 扩展可以使用本机消息功能与配套的通用 Windows 平台 (UWP) 应用进行通信。  在高级别上，Microsoft Edge 扩展将相同的 Api 用于本机消息作为 Chrome 和 Firefox 扩展。 但是，将需要使用通用 Windows 平台来实现本机消息主机。

> [!NOTE]
> 以下概述 (通过 AppService) 连接到 UWP 应用的方法是支持 Microsoft Edge 扩展和本机组件之间的通信的唯一支持机制。 有关如何启用与旧版 Win32 组件的通信的详细信息，请参阅本指南的 " [添加桌面桥组件](#adding-a-desktop-bridge-component) " 部分。 
...
你还需要设置允许哪些扩展 (s) 连接到该服务。 由于 Microsoft Edge `"allowed_origins"` 在其 package.appxmanifest 中没有等效的清单属性，因此需要在运行时由 UWP 应用确定并强制执行此操作。 由于 Microsoft Edge 将代表扩展建立连接，因此应用可以查找呼叫者的程序包系列名称，以确定它们是否通过 Microsoft Edge 进行连接以控制或验证呼叫者。 例如 
...
对于 Microsoft Edge，虽然对 `AppService` 邮件大小没有限制 (依赖于内存) ，但 Microsoft Edge 通过强制执行下列邮件大小限制来防止自身对本机应用的行为：
...
对延迟而言，应用服务使用 "命名管道"，在初始连接后，允许两个应用直接通信。 这种通信方式产生较低的延迟。 Cpu 速度较慢的设备将在启动承载应用服务的进程后遇到一些初始延迟 (~ 80ms 以启动某些设备上的后台任务) 。 启动后，CPU 设备速度较慢的性能应该很好。 
...
7. 生成并 [部署](#deploying) UWP 项目 (，其中包含可选的桌面桥组件) 。
...
## 部署
配置 UWP 项目 (和可选 Win32 项目) 如上所述，准备好使用 Visual Studio 部署解决方案。
