...
> [!NOTE]
> 如果你仅下载 Windows 10 SDK 以使用 MakePri.exe 工具，你可以仅选择 "适用于桌面应用的 Windows SDK 签名工具" 和 "适用于 UWP 托管应用的 Windows SDK" 功能以使下载更浅。 MakePri.exe 工具将出现在 C:\Program 文件的子文件夹中， (x86) \Windows Kits\10\bin\10.0.17713.0。
...
若要指定你的扩展支持的语言的列表，你可以 `Resource` 使用下面所示的格式添加元素 (此 `Resource` 元素将显示 Microsoft Store) 中对英语、德语和法语的支持：
...
在 Microsoft Edge 外部、Windows 和 Microsoft Store 中，唯一支持的本地化框架基于通用 Windows 平台 (UWP) 本地化框架。
...
首先，你需要创建配置文件。 这将为资源定义默认的限定符和平台。 在此示例中，将默认语言 " (US) 和平台 Windows 10 设置为英语。 若要执行此操作，请在 [Root 文件夹] 中创建具有以下内容的 priconfig.xml 文件：
...
如果你仅有兴趣为 Microsoft Store (而不是 Windows) 提供本地化名称和说明，则可以通过 [保留你的扩展的所有本地化名称](./extensions-in-the-windows-dev-center.md#name-reservation)来执行此操作。
