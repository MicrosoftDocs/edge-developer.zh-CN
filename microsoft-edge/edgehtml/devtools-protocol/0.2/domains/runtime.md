...
---
# <a name="runtime-domain---devtools-protocol-version-02-edgehtml"></a>运行时域 - DevTools 协议版本 0.2 (EdgeHTML)   
...
启用对 `executionContextCreated` 、 `executionContextDestroyed` 和事件 `executionContextsCleared` 的报告。  启用报告后， `executionContextCreated` 将立即针对每个现有执行上下文发送事件。  

---  

### <a name="disable"></a>“禁用”  

禁用报告 、 `executionContextCreated` `executionContextDestroyed` 和 `executionContextsCleared` 事件。  
...
|:--- |:--- |:--- |  
| 表达式 | `string` | 要计算表达式。 |  
| includeCommandLineAPI \ (optional\)  | `boolean` | 确定在评估期间命令行 API 是否可用。 |  
| objectGroup \ (optional\)  | `string` | 可用于释放多个对象的符号组名称。 |  
...
| boolean \ (optional\)  | `boolean` | 在静默模式下，不会报告在评估期间引发的异常，并且不会暂停执行。 覆盖 `setPauseOnException` 状态。 |  
| returnByValue \ (optional\)  | `boolean` | 结果是否应为应按值发送的 JSON 对象。 |  
| awaitPromise \ (optional\)  | `boolean` | 是否应为 `await` 生成的值执行，并解决等待的承诺后返回。 |  
| executionContextId \ (optional\)  | [ExecutionContextId](#executioncontextid) | 指定将用来调用函数的全局对象的执行上下文。  任一
`executionContextId` 或 `objectId` 应指定 |  
| objectGroup \ (optional\)  | `string` | 可用于释放多个对象的符号组名称。  如果 `objectGroup` 未指定且 `objectId` 为， `objectGroup` 则继承自对象。 |  
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| promiseObjectId | [RemoteObjectId](#remoteobjectid) | 承诺的标识符。 |  
| returnByValue \ (optional\)  | 布尔型 | 结果是否应为应按值发送的 JSON 对象。 |  

| 返回 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| result | [RemoteObject](#remoteobject) | 承诺结果。  如果承诺被拒绝，将包含拒绝的值。 |  
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| objectId | [RemoteObjectId](#remoteobjectid) | 要释放的对象的标识符。 |  
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| 上下文 | [ExecutionContextDescription](#executioncontextdescription) | 新建的执行上下文。 |  
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| executionContextId | [ExecutionContextId](#executioncontextid) | 已销毁上下文的 ID。 |  
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| 类型 | `string` | 呼叫的类型。  允许的值 `log` `info` `warning` ：、、、、、、、、、 `error` `debug` `assert` `table` `trace` `dir` `dirxml` `clear` `select` `count` `countReset` `timeEnd` `timeStamp` `startGroup` `startGroupCollapsed` `endGroup` |  
| args | [RemoteObject[]] (#remoteobject | 调用参数。 |  
| executionContextId | [ExecutionContextId](#executioncontextid) | 进行控制台调用的上下文的标识符。 |  
| timestamp \ (optional\)  | [时间戳](#timestamp) | 调用时间戳。 |  
| stackTrace \ (optional\)  | [StackTrace](#stacktrace) | 堆栈跟踪捕获（如果可用）。 |  
...
| subtype \ (optional\)  | `string` | 对象子类型提示。  仅为 `object` 类型值指定。  允许的值  `null` `error` `promise` ：、、和 `node` |  
...
| 可写 \ (可选\)  | `boolean` | `True` 如果与该属性关联的值可能已更改 \ (数据描述符\) 。 |  
| 获取 \ (optional\)  | [RemoteObject](#remoteobject) | 充当属性 getter 的函数，或者如果没有 `undefined` getter \ (访问器描述符\) 。 |  
| set \ (optional\)  | [RemoteObject](#remoteobject) | 充当属性的设置器的函数，或者如果没有设置器 `undefined` \ (访问器描述符\) 。 |  
...
代表函数调用参数。  应指定远程对象 ID、基元、不可erializable 基元值 (`objectId` `value` \) 的 \) 。  
...
### <a name="executioncontextdescription-object"></a>ExecutionContextDescription 对象  
...
| 属性 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| id | [ExecutionContextId](#executioncontextid) | 执行上下文的唯一 ID。  它可用于指定执行上下文
应执行脚本评估。 |  
| origin | `string` | 执行上下文源。 |  
| name | `string` | 描述给定上下文的可读名称。 |  
...
| scriptId | [ScriptId](#scriptid) | JavaScript 脚本 ID。如果未启用调试器，ScriptId 将为空。 |  
| url | `string` | JavaScript 脚本名称或 url。 |  
| lineNumber | `integer` | JavaScript 脚本行号 \ (0-based\) 。 |  
| columnNumber | 整数 | JavaScript 脚本列号 \ (0-based\) 。 |  
