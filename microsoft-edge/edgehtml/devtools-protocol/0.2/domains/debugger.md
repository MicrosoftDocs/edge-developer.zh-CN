...
---
# <a name="debugger-domain---devtools-protocol-version-02-edgehtml"></a>调试器域 - DevTools 协议版本 0.2 (EdgeHTML)   
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| state | `string` | 暂停异常模式。  允许的值：  `none` `uncaught` ， ， `all` |  
...
**实验**性 。  将以前的黑箱模式替换为传递的黑色框模式。  强制后端在 URL 与其中一种模式匹配的脚本中跳过单步执行/暂停。  调试器将尝试通过多次执行"分步"来退出黑盒脚本，如果失败，最后会使用"退出"。  
...
| 参数 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| debuggerPropertyId | 字符串 | Microsoft：要设置 (`msDebuggerPropertyId` \) \ 属性 ID。 | 
| newValue | `string` | &nbsp; |  
...
分析脚本时触发。 启用调试程序时，还会针对所有已知和未安装脚本触发此事件。  
...
| reason | `string` |  暂停原因。  允许的值  `breakpoint` `step` `exception` ：、、、 `other` 和 `EventListener` |  
| data \ (optional\)  | `object` | 包含特定于中断的辅助属性的对象。 |  
| hitBreakpoints \ (optional\)  | `string[]` | 命中断点 ID |  
| asyncStackTrace \ (optional\)  | `StackTrace` | JavaScript 异步堆栈跟踪。 |  
...
| msLength | `integer` | Microsoft：代码 \ (，即此调用帧) 字符数\字符数。 |  
...
| columnNumber \ (optional\)  | `integer` | 脚本 \ (0\) 中的列号。 |  
| msLength | `integer` | Microsoft：代码 \ (，即此调用帧) 字符数\字符数。 |  
| type \ (optional\)  | `string` | 允许的值：debuggerStatement、call、return。 |  
...
| 属性 | 类型 | 详细信息 |  
|:--- |:--- |:--- |  
| 类型 | `string` | 范围类型。  允许的值  `global` `local` `with` ：、、、、、 `closure` `catch` `block` `script` `eval` `module` 和 `return` |  
| object | [Runtime.RemoteObject](./runtime.md#remoteobject) | 表示范围的对象。  对于它表示实际对象和范围;对于其余范围，它是人工临时对象枚举范围变量 `global` `with` 作为其属性。 |  
