...
2. **导入分析会话（Ctrl + O）**：加载保存的 DevTools 内存诊断会话。
3. **导出分析会话（Ctrl + S）**：将当前的诊断会话保存到磁盘。
4. **获取堆快照（Ctrl + Shift + T）**：记录给定时间点的当前内存分配。
...
启动内存探查器时，你将看到一个进程内存图，使你能够观察整个专用工作集（页面按时间使用的内存量）。 "内存" 图显示了选项卡的进程内存的实时视图，其中包括专用字节、本机内存和 JavaScript 堆。 
...
单击摘要磁贴中的值将切换到显示[快照数据详细信息](#snapshot-details)的窗格。 潜在的[内存问题](#snapshot-details)用蓝色信息（"i"）图标表示。
...
列 | 描述
:------------ | :-------------
标识符 | 最能识别对象的名称。 例如，对于 HTML 元素，快照详细信息显示 ID 属性值（如果使用了一个）。
...
循环引用以星号（*）和信息性工具提示显示，并且不能展开。 否则，它们将阻止你向上遍历引用树和标识保留内存的对象。

若要快速识别等效对象，请勾选 "[*显示对象 id*](#filters) " 筛选器选项，以在 "*标识符*" 列中的对象名称旁边显示对象 id。 具有相同 ID 的对象为共享引用。
...
- <strong>"快照 #：" 中的所有对象都 <number></strong> 显示堆上的所有对象（换言之，即未 <em> 筛选的 </em> 视图）。
