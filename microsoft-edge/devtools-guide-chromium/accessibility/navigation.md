...
例如 **，Elements** 工具包含用于检查事件侦听器、辅助功能树等的其他选项卡。  选项卡和窗格之间的区别在一定程度上是任意的。  可能审阅某一个术语的唯一原因只是为了与官方开发工具文档的其余部分保持一致。  
...
## <a name="navigate-between-panels"></a>在面板之间导航  
...
*   一旦激活每个面板，**某些**面板（**** 如控制台和性能工具）可能会将焦点移到面板内容区域中。  这可能使得使用箭头键导航更加困难。  
...
完成以下操作以打开 **Elements** 工具。  

1.  打开“**命令菜单**”。  
1.  键入 `E` ，然后键入 `L`。  选择“**面板>显示元素**”选项。  
1.  选择 `Enter` 以运行命令打开面板。  

用这种方式打开一个面板会将焦点引导到面板的内容上。  对于 Elements 工具 **，焦点** 将移动到 **DOM 树中**。  

## <a name="elements-panel"></a>元素面板  
...
## <a name="elements-tool-panels"></a>元素工具面板  

元素 **工具** 包含用于检查应用于元素或辅助功能树中相关位置的 CSS 等内容的其他选项卡。  
...
### <a name="event-listeners-tab"></a>事件侦听器选项卡  

在" **元素** "工具中，可以使用"事件侦听器"选项卡检查应用于元素 **的事件** 侦听器。 焦点在 **"样式"** 面板上时，选择以 `Right Arrow` 导航到 **"事件侦听器"** 面板。  
...
### <a name="accessibility-pane"></a>辅助功能窗格  
...
## <a name="audits-panel"></a>“审核”面板  

审核 **工具** 应针对网站运行一系列测试，以检查与性能、辅助功能、SEO 和大量其他类别相关的常见问题。  

### <a name="configure-and-run-an-audit"></a>配置和运行审核  

1.  首次**打开审核**工具时，焦点将放在表单末尾的"运行**** 审核"按钮上。  默认情况下，窗体配置为使用模拟 3G 连接上的移动仿真对每个类别运行审核。  
...
### <a name="navigate-the-audit-report"></a>导航审核报告  
