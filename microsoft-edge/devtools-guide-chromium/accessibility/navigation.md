...
用这种方式打开一个面板会将焦点引导到面板的内容上。  对于" **元素"工具** ，焦点将移动到 **DOM 树中**。  
...
在" **元素"** 工具中，可以使用"事件侦听器"选项卡检查应用于 **元素的事件** 侦听器。 焦点在" **样式"** 面板上时，选择 `Right Arrow` 以导航到" **事件侦听器"** 面板。  
...
**审核工具**应针对网站运行一系列测试，以检查与性能、辅助功能、SEO 和大量其他类别相关的常见问题。  

### <a name="configure-and-run-an-audit"></a>配置和运行审核  

1.  首次**打开审核**工具时，焦点放置在表单末尾的"运行审核****"按钮上。  默认情况下，窗体配置为使用模拟 3G 连接上的移动仿真对每个类别运行审核。  
