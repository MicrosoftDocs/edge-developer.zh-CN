...
# DevTools 中的新增功能（Microsoft Edge 81）   
...
我们正在处理 UI 并向3D 视图添加更多功能，因此请向我们发送[反馈](#feedback)。  
...
> ##### 图 5  
> VS 代码中的元素工具使用 Microsoft Edge 扩展的元素 ![ 使用适用于 Microsoft edge 扩展的元素的 Vs 代码中的元素工具][ImageElementsVisualStudioCode]  
...
*   打开[命令菜单][CommandMenu]并运行 `Capture screenshot` 命令以获取包含 Moto G4 硬件的视口的屏幕截图（在启用 "**显示设备框架**" 之后）。  
...
Chromium 问题[#1026879][crbug1026879]  
...
#### 复制为 Node fetch 以包括 cookie 数据   

右键单击网络请求并选择 "**复制**  >  **副本" 作为节点。 js fetch**获取 `fetch` 包含 cookie 数据的表达式。  
...
> 原始页面位于[此处](https://developers.google.com/web/updates/2020/01/devtools/index)，由[Kayce Basques][KayceBasques] \ （技术作者、Chrome DevTools & Lighthouse \）创作。  
