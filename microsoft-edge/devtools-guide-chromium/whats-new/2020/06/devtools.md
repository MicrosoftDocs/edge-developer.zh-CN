...
---
# DevTools 中的新增功能（Microsoft Edge 85）  
...
现在，你可以使用**网络控制台**在[网络日志][DevtoolsNetworkIndexLogActivity]中使用 "**编辑和重播**" 请求。  
...
新的面板，**网络控制台**将在[DevTools 抽屉][DevtoolsCustomizeIndexDrawer]中打开，并自动填充 HTTP 请求的信息。  若要查看从服务器返回的响应，请编辑请求 \ （如果需要 \），然后选择 "**发送**"。  
...
> [!TIP]
> 若要在主 DevTools 面板（而不是[抽屉][DevtoolsCustomizeIndexDrawer]）中查看**网络控制台**，请参阅[在面板之间移动工具](#move-tools-between-panels)。  
...
通常，**元素**和**网络**之类的工具只能在 DevTools 的 main \ （top \）面板中打开。  同样， **3D 视图**和**问题**之类的工具可能仅在 DevTools 的抽屉 \ （底部 \）面板中打开。  现在，你可以通过在顶部面板和底部面板之间移动工具来自定义 DevTools 布局。  
...
现在，"**样式**" 窗格对通过[CSS 对象模型（CSSOM）][CsswgDraftsCssom] api 创建的样式有更好的支持。  许多 CSS 中的 CSS 框架和库使用 CSSOM Api 来构建样式。  
...
例如， `h1` 通过 `CSSStyleSheet` \ （CSSOM api \）添加的样式先前不可编辑。  样式现在可在 "**样式**" 窗格中编辑。  
...
Lighthouse 6.0 将三个新度量值引入到报表：最大的 Contentful 油漆 \ （LCP \）、累积布局班次 \ （CLS \）和总阻塞时间 \ （TBT \）。  
...
第一个有意义的油漆 \ （FMP \）在 Lighthouse 6.0 中已弃用。  FMP 也已从 "**性能**" 面板中删除。  **最大 Contentful 画图**是推荐的 FMP 替换。  <!--See [First Meaningful Paint][WebDevFirstMeaningfulPaint] for an explanation of why it was deprecated.  -->  
...
> 原始页面位于[此处](https://developers.google.com/web/updates/2020/06/devtools/index)，由[Jecelyn Yeen][JecelynYeen] \ （开发人员和 Chrome DevTools \）创作。  
