...
**CSS** 是决定网页布局和样式的计算机语言。  下图是带边框的段落。  
...
1.  将以下代码添加到 `contact.html`。  请记住，需要添加以 `<style>` 开始到 `</style>` 结尾的行。  其他代码就在这里，以便你了解新代码的放入位置。  
...
1.  在 `<nav>` 中 `<main>` 包 `<div class="row">`元素。  请务必在 `</div>` 之后 `</main>` 添加 ，以便正确关闭新标记。  
...
*   若要深入了解如何使用 DevTools 尝试页面的 CSS，请导航到 ["开始查看和更改 CSS 管理][DevtoolsCssIndex]。  
