...
1.  [Open DevTools][DevToolsOpen]按 `Control` + `Shift` + `J` \ (Windows、Linux \ ) 或 `Command` + `Option` + `J` \ (macOS \ ) 打开 DevTools。  此时将打开 " **控制台** " 面板。  
...
1.  键入 `Cache-Control` 并选择 `Enter` 。  "搜索" 窗格 `Cache-Control` 会列出它在 "资源标题" 或 "内容" 中找到的所有实例。  
...
1.  选择 " **筛选** \ (![ 筛选 ][ImageFilterIcon] \ ) " 以显示它。  
...
1.  若要查看脚本，请按住 `Control` (Windows、Linux \ ) 或 `Command` \ (macOS \ ) ，然后选择 " **JS**"。  
...
导航到 [网络引用][DevtoolsNetworkReference] 以发现更多与检查网络活动相关的 DevTools 功能。  
