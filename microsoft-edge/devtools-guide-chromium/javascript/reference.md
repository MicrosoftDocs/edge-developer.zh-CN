...
若要了解如何设置断点，请导航到"使用断点[暂停代码"。][DevToolsJavascriptBreakpoints]  

## <a name="step-through-code"></a>逐步执行代码  

暂停代码后，一次分步执行一行，一直调查控制流和属性值。  

### <a name="step-over-line-of-code"></a>逐行代码  

当暂停包含与正在调试的问题不相关的函数的代码行时，选择"单步执行 **\ (** 单步 ![ 执行 \) "按钮以运行该函数，而无需单步 ](../media/step-over-icon.msft.png) 执行。  
...
你已暂停 `A` 。  选择"单步 **执行"** 后，DevTools 将运行要单步执行的函数中的所有代码，即 `B` 和 `C` 。  DevTools 随后在 上暂停 `D` 。  

### <a name="step-into-line-of-code"></a>逐行执行代码  

暂停包含与正在调试的问题相关的函数调用的代码行时，选择"单步执行 **\ (** 单步执行 ![ \) "按钮以进一步调查该 ](../media/step-into-icon.msft.png) 函数。  
...
你已暂停 `A` 。  选择"进入 **"后**，DevTools 将运行此代码行，然后在 上暂停 `B` 。  

### <a name="step-out-of-line-of-code"></a>退出代码行  
...
你已暂停 `A` 。  选择" **步骤"后**，DevTools 将运行 中代码的其余部分，仅在此例中， `getName()` 然后在 `B` 上暂停 `C` 。  

### <a name="run-all-code-up-to-a-specific-line"></a>运行所有代码，最多运行特定行  

调试长函数时，可能有许多代码与正在调试的问题不相关。  

你可以选择逐行执行，但这很繁琐。  您可以选择在感兴趣的行上设置代码行断点，然后选择"Resume **Script Execution** \ (Resume Script ![ Execution \) "按钮，但有一个更快的方法 ](../media/resume-script-run-icon.msft.png) 。  

将鼠标悬停在您感兴趣的代码行上，打开上下文菜单 \ (右键单击\) ，然后选择"继续**在此处"。**  DevTools 运行所有代码，一直运行到该点，然后暂停到该行。  
...
你已暂停 `A` 。  选择"**重启帧**"后，应暂停在 上，而不会设置断 `B` 点或选择 **"恢复脚本执行"。**  
...
若要在脚本暂停后继续运行时，请选择"恢复脚本执行 **\ (** ![ Resume Script Execution ](../media/resume-script-run-icon.msft.png) \) "按钮。  DevTools 将一直运行脚本，直到下一个断点（如果有）。  
...
例如，假设您在主脚本和服务工作线程脚本的断点上暂停。  您希望查看服务工作线程上下文的本地和全局属性，但"源"面板显示**** 主脚本上下文。  通过选择"线程"窗格中的服务工作器**** 条目，应该能够切换到该上下文。  
...
在代码行上暂停时，使用"调用堆栈****"窗格查看调用堆栈，该堆栈可让你到达此点。  
...
当你想要在调试时忽略该脚本时，将脚本标记为库代码。  标记为库代码时，脚本在"调用堆栈"窗格中被**** 遮盖，并且你在单步执行代码时绝不会单步执行脚本的功能。  
...
### <a name="mark-a-script-as-library-code-from-the-editor-pane"></a>从"编辑器"窗格中将脚本标记为库代码  

完成以下操作，从"编辑器"窗格中 **将脚本标记为** 库 **代码** 。  
...
1.  选择 **"标记为库代码"。**  
    ...
    :::image type="complex" source="../media/javascript-glitch-debug-js-sources-get-started-inputs-are-empty-call-stack-mark-as-library-code.msft.png" alt-text="从"调用堆栈"窗格中将脚本标记为库代码" lightbox="../media/javascript-glitch-debug-js-sources-get-started-inputs-are-empty-call-stack-mark-as-library-code.msft.png":::
       从"调用 **堆栈"窗格中将** 脚本 **标记为库** 代码  
    :::image-end:::  
    
### <a name="mark-a-script-as-library-code-from-settings"></a>从"设置"将脚本标记为库代码  

完成以下操作以从"设置"中标记单个脚本或 **脚本模式**。  
...
*   选择" **刷新** \ (![ Refresh ](../media/refresh-icon.msft.png) \) "按钮以刷新所有现有表达式的值。  逐步执行代码时，值将自动刷新。  
...
修复 Bug 时，通常需要测试对 JavaScript 代码的一些更改。  无需在外部编辑器或 IDE 中进行更改，然后刷新页面。  你可以编辑 DevTools 中的脚本。  

完成以下操作以编辑脚本。  

1.  在"源"面板 **的"编辑器** "窗格中 **打开** 文件。  
1.  在"编辑器" **窗格中进行更改** 。  
1.  选择要 `Ctrl` + `S` 保存 (Windows、Linux\) 或 `Command` + `S` \ (macOS\) \ 。  DevTools 将整个 JS 文件修补到 Microsoft Edge 的 JavaScript 引擎中。  
