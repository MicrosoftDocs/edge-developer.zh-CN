...
当你知道需要调查的确切代码区域时，请使用代码行断点。  在运行此行代码之前，DevTools**始终**暂停。  
...
> ##### 图 3  
> 显示两个代码行断点的 "**断点**" 面板：一行位于 `16` `get-started.js` ，另一行 `33`  
...
> ##### 图 6  
> 在**XHR 断点**中为包含 `org` 在 URL 中的任何请求创建 XHR 断点  
...
1.  单击 "**源**" 选项卡。  
1.  单击 "异常暂停时**暂停**" ![ 异常 ][ImagePauseOnExceptionsIcon] 。  启用时图标变为蓝色。  
...
`debug(method)`如果要在运行特定函数时暂停，请运行该方法，其中 `method` 是要调试的命令、函数或方法。  你可以插入 `debug()` 代码（如 `console.log()` 语句）或从 DevTools 控制台运行该方法。  `debug()` 等效于在函数的第一行上设置[代码行断点](#line-of-code-breakpoints)。  
...
> 原始页面位于[此处](https://developers.google.com/web/tools/chrome-devtools/javascript/breakpoints)，由[Kayce Basques][KayceBasques] \ （技术作者、Chrome DevTools & Lighthouse \）创作。  
