...
1.  在 `5` " **数字 1"文本框** 中输入。  
1.  在 `1` " **数字 2"文本框** 中输入。  
1.  选择 **"添加数字 1 和数字 2"。**  按钮下方的标签显示 `5 + 1 = 51` 。  结果应为 `6` 。  接下来，修复 Bug 的添加错误。  
...
DevTools 为不同的任务提供了许多不同的工具。  不同的任务包括更改 CSS、分析页面加载性能和监视网络请求。  源 **工具** 是调试 JavaScript 的地方。  
...
1.  文件 **导航器** 面板 \ (上图\中第 1 节) 。  此处列出了网页请求的所有文件。  
1.  上 **图中的代码** 编辑器面板\ (第 2 部分\) 。  在"文件导航器"**** 窗格中选择文件后，该文件的内容会显示在此处。  
1.  上 **图中的 JavaScript** 调试面板 \ (第 3 部分\) 。  用于检查网页的 JavaScript 的各种工具。  如果 DevTools 窗口很宽，则此窗格显示在代码编辑器 **窗格的右侧** 。  
    
## <a name="step-3-pause-the-code-with-a-breakpoint"></a>步骤 3：使用断点暂停代码  

调试这种类型的问题的常见方法是在代码中插入多个语句，然后在脚本运行时 `console.log()` 检查值。  例如：  
...
`console.log()`该方法可能会完成工作，但断点可以更快地**** 完成。  断点允许你在运行时中间暂停代码，并检查此时的所有值。  断点与方法相比具有以下 `console.log()` 优点。  
...
*   在 `console.log()` 语句中，需要显式指定要检查的每个值。  使用断点时，DevTools 会显示此时所有变量的值。  有时，影响代码的变量被隐藏和混淆。  
    
简而言之，断点可帮助您更快地查找和修复 `console.log()` Bug。  

如果你后退并思考应用的工作原理，你可能会有根据地猜测错误的总和 \ (\) 是在与"添加数字 `5 + 1 = 51` 1"和"数字 `click` **2"** 按钮关联的事件侦听器中计算的。  因此，你可能想要在侦听器运行时暂停 `click` 代码。  **事件侦听器断点** 让你可以完全实现此要求：  
...
1.  选择旁边的复选框以 **单击**。  DevTools 现在设置为在任何事件侦听器运行时 `click` 自动暂停。  
...
1.  返回到演示，再次选择"**添加数字 1 和数字 2"。**  DevTools 暂停演示，并突出显示源工具中的一 **行** 代码。  DevTools 应在 第 16 行中暂停 `get-started.js` 。  
    
    ```javascript
    if (inputsAreEmpty()) {
    ```  
    
    如果暂停另一行代码，请选择"恢复脚本 **执行** "\ ("恢复脚本执行 ![ ][ImageResumeIcon] ") 直到您暂停正确的行。  
    
    > [!NOTE]
    > 如果您在另一行上暂停，则具有浏览器扩展，该扩展在访问的每一个网页上注册 `click` 事件侦听器。  在扩展的 `click` 侦听器中暂停。  如果使用 InPrivate 模式以**** 专用方式浏览，这将禁用所有扩展，你可能会看到每次在所需的代码行上暂停。  
...
出现 Bug 的一个常见原因是脚本按错误的顺序运行。  单步执行代码允许你演练代码的运行时。  一次演练一行，帮助你准确了解代码以不同于预期的顺序运行位置。  立即尝试：  
...
1.  在**** DevTools 的"源"工具上，选择"单步执行下一个函数调用**\ (** 单步执行下一个函数调用 \) 以单步执行函数的运行时，一次一行 ![ ][ImageIntoIcon] `updateLabel()` 。  
    
一次查看一行是逐步执行代码的基本概念。  如果您查看中的代码 `get-started.js` ，则 bug 可能位于函数 `updateLabel()` 中的某一位置。  与其逐行执行每行代码，不如使用另一种类型的断点来暂停代码，以更接近 bug 的可能位置。  
...
1.  在左侧，此特定代码行的编号显示为**34。**  选择第 **34 行**。  DevTools 在 **34**的左侧显示一个红色图标。  红色图标表示代码行断点位于此行上。  DevTools 始终在运行此代码行之前暂停。  
1.  Choose **Resume script execution** \ (Resume script execution ![ ][ImageResumeIcon] \) .  脚本将继续运行，直到到达第 33 行。  在行 31、32 和 33 上，DevTools 在每行上打印分号的和右边 `addend1` `addend2` `sum` 的值。  
...
的值 `addend1` ， `addend2` 看起来 `sum` 可疑。  这些值用引号括起来。  引号表示值是一个字符串，这是一个很好的假设，可以解释 Bug 的原因。  收集有关情况详细信息。  DevTools 提供了许多用于检查变量值的工具。  
...
使用 **监视表达式** 面板，可以随着时间的推移监视变量的值。  正如名称所示， **监视表达式** 不限于变量。  你可以将任何有效的 JavaScript 表达式存储在 **监视表达式中**。  立即尝试。  
...
> [!NOTE]
> 在 **"监视表达式** "窗格 \ (右下\) ，将显示 `typeof sum` 监视表达式。  如果 DevTools 窗口较大，则 **监视表达式** 窗格位于事件侦听器断点窗格 **的** 右侧。  
...
正如所怀疑的，当字符串应为数字时 `sum` ，它将作为字符串进行求值。  现在确认值类型是 Bug 的原因。  

### <a name="method-3-the-console"></a>方法 3：控制台  

控制台 **允许你** 查看消息，还可以使用它来评估 `console.log()` 任意 JavaScript 语句。  对于调试，可以使用 **控制台** 测试潜在 Bug 修复。  立即尝试。  

1.  如果 **控制台工具** 已关闭，请选择 `Escape` 将其打开。  控制台 **工具** 在 DevTools 窗口的下面板中打开。  
1.  在 **控制台中**，键入 `parseInt(addend1) + parseInt(addend2)` 。  工具的语句暂停在一行代码（其中 `addend1` 和 `addend2` 位于范围内）。  
1.  选择 `Enter`。  DevTools 计算语句并打印，这是预期演示 `6` 生成的结果。  
    ...
    :::image type="complex" source="../media/javascript-sources-breakpoint-paused-console.msft.png" alt-text="控制台工具，在评估 parseInt (addend1) + parseInt (addend2) " lightbox="../media/javascript-sources-breakpoint-paused-console.msft.png":::
       在 **评估** 后，控制台工具 `parseInt(addend1) + parseInt(addend2)`  
    :::image-end:::  
    
## <a name="step-7-apply-a-fix"></a>步骤 7：应用修补程序  

如果找到 Bug 的修复程序，请通过编辑代码并重新运行演示来尝试修复。  可以直接在 DevTools UI 中编辑 JavaScript 代码并应用修补程序。  立即尝试。  
...
1.  Choose **Deactivate breakpoints** \ (![ Deactivate breakpoints ][ImageDeactivateIcon] \) .  它更改蓝色以指示该选项处于活动状态。  在 **设置 Deactivate 断点时** ，DevTools 将忽略你设置的任何断点。  
...
恭喜你！  现在，你已了解如何在调试 JavaScript 时充分利用 Microsoft Edge DevTools。  本文中学习的工具和方法可能会为您节省数百个小时。  

本文只介绍了两种设置断点的方法。  DevTools 提供了许多其他方法，包括以下设置。  

*   仅在您提供的条件为 true 时触发的条件断点。  
*   捕获或不捕获的异常的断点。  
*   当请求的 URL 与提供的子字符串匹配时触发的 XHR 断点。  
    
有关何时以及如何使用每种类型，请导航到"使用断点[暂停代码"。][DevtoolsJavscriptBreakpoints]  
