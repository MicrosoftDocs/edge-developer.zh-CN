...
1.  在 "**引用当前选定的包含 $0 的节点**" 下，右键单击**暗度的左侧**，然后选择 "**检查**"。  
    ...
    1.  单击 `<li>Dune</li>` DOM 树， `$0` 再次键入控制台，然后再次按下 `Enter` 。  现在， `$0` 计算结果为 `<li>Dune</li>` 。  
        ...
        > ##### 图 15  
        > 控制台中第二个 `$0` 表达式的结果是 ![ 控制台中第二个 $0 表达式的结果][ImageSecondConsole]  
...
本节将快速介绍 HTML 和 DOM 之间的区别。  

使用 web 浏览器请求页面时，服务器将返回 HTML，如下所示：  
...
表示页面内容的此对象树（即表示页面内容的节点）称为 DOM。  现在，它看起来与 HTML 相同，但假设 HTML 底部引用的脚本运行此代码：  
...
该代码将删除 `h1` 节点，并将另一个 `p` 节点添加到 DOM。  完整的 DOM 现在如下所示：  
...
> 原始页面位于[此处](https://developers.google.com/web/tools/chrome-devtools/dom/index)，由[Kayce Basques][KayceBasques] \ （技术作者、Chrome DevTools & Lighthouse \）创作。  
