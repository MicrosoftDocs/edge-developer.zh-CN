...
用户期望交互式和流畅的页面。  像素管道中的每个阶段都表示引入 jank 的机会。  了解识别和修复减慢运行时性能的常见问题的工具和策略。  
...
JavaScript 计算（尤其是触发大量视觉更改的计算）可能会阻碍应用程序性能。  不要让时间不佳或长时间运行的 JavaScript 干扰用户交互。  

### JavaScript：工具  

在"性能"面板 **中** 录制并查找可疑的长 `Evaluate Script` 事件。  <!--If you find any, you are able to enable the **JS Profiler** and re-do your recording to get more detailed information about exactly which JavaScript functions were used and how long each took.  -->  

<!--todo: add Recording section when available  -->  
<!--todo: add Profile JavaScript (JS Profiler) section when available  -->  

当你在 JavaScript 中十分明显时，你可能需要将你的分析介绍到下一个级别并收集 JavaScript CPU 配置文件。  CPU 配置文件显示运行时在页面功能中的使用位置。  了解如何在快速 [JavaScript][DevtoolsRenderingToolsJavascriptRuntime]运行时创建 CPU 配置文件。
...
| 问题 | 示例 | 解决方案 |  
|:--- |:--- |:--- |  
| 影响响应或动画的成本高昂的输入处理程序。  | 触摸、视差滚动。  | 让浏览器处理触摸和滚动，或尽可能晚地绑定侦听器。  请参阅 [Paul 的运行时性能清单中的高成本输入处理程序][WebPerformanceCalendarRuntimeChecklist]。  |  
| 时间太重的 JavaScript 影响响应、动画、加载。  | 用户在页面加载后向右滚动，setTimeout/setInterval。  | 优化 JavaScript 运行时：使用 `requestAnimationFrame` ，在帧上分布 DOM 操作，使用 [Web 工作人员][MDNUsingWebWorkers]。  |  
| 影响响应的长时间运行的 JavaScript。  | [DOMContentLoaded 事件][MDNUsingWebWorkers]因使用 JS 工作而停止。  | 将纯计算工作移动到 [Web 工作人员][MDNUsingWebWorkers]。  如果需要 DOM 访问权限，请使用 `requestAnimationFrame` 。  <!--See also [Optimize JavaScript Execution][WebFundamentalsPerformanceRenderingOptimizeJavascriptRuntime].  -->  |  
| 影响响应或动画的垃圾脚本。  | 垃圾收集可能在任何位置发生。  | 编写更少的垃圾脚本。  请参阅 Paul Paul 的运行时性能清单中的 [动画中的垃圾回收][WebPerformanceCalendarRuntimeChecklist]。  |  

<!--todo: add Optimize JavaScript runtime section when available  -->  

## 样式  

样式更改非常昂贵，尤其是在这些更改影响 DOM 中的多个元素时。  每次向元素应用样式时，浏览器都会指出对所有相关元素的影响、重新计算布局和重画。  
...
在"性能"面板 **中录制** 。  检查录制大事件 `Recalculate Style` \ (以紫色\) 。  

<!--todo: add Recording section when available  -->  

单击事件 `Recalculate Style` ，在"详细信息"窗格中查看有关 **该事件** 的详细信息。  如果样式更改需要很长时间，则性能会下降。  如果样式计算影响大量元素，则这是另一个有改进空间的区域。  
...
Firefox (或重排) 浏览器计算页面上所有元素的位置和大小的过程。  Web 的布局模型意味着一个元素可能会影响其他元素;例如，元素的宽度通常影响任何子元素的宽度，等等，一直向上和向下影响 `<body>` 树。  浏览器可能涉及此过程。  
...
"布局线"是强制同步布局条件的重复。  当 JavaScript 重复写入和读取 DOM 时，会出现此情况，这会强制浏览器反复重新计算布局。  若要标识布局限制，请查找多个强制同步布局警告的模式。  请参阅上图。  
...
| 问题 | 示例 | 解决方案 |  
|:--- |:--- |:--- |  
| 影响响应或动画的强制同步布局。  | 强制浏览器在像素管道中更早地执行布局，从而在呈现过程中重复步骤。  | 首先批处理样式读取，然后执行任何写入操作。  <!--See also [Avoid large, complex layouts and layout thrashing][WebFundamentalsPerformanceRenderingAvoidLargeComplexLayouts].  -->  |  
...
绘制是填充像素的过程。  它通常是呈现过程成本最大的部分。  如果你注意到你的页面以任何方式都是不稳定的，你很可能遇到绘制问题。  

合成是页面的绘制部分组合在一起以在屏幕上显示的地方。  大多数情况下，如果你坚持使用仅合成器属性，并且完全避免绘制，则应该会看到性能的明显改进，但需要注意层计数过多。  <!--See also [Stick to compositor-only properties and manage layer count][WebFundamentalsPerformanceRenderingCompositorOnlyProperties].  -->  

<!--todo: add Stick to compositor-only properties and manage layer count section when available  -->  

### 绘制和复合：工具  

想知道绘制需要多久或多久进行一次绘制？  检查 ["性能"面板][DevtoolsChromiumEvaluatePerformanceReferenceEnableadvancedpaintinstrumentation] 中的" **启用高级绘制** 检测"设置，然后录制。  如果大多数呈现时间都用于绘制，则存在绘制问题。  
...
| 问题 | 示例 | 解决方案 |  
|:--- |:--- |:--- |  
| 影响响应或动画的绘制风暴。  | 影响响应或动画的大画区或昂贵的画图。  | 避免绘制、升级要移动到其自己的图层的元素、使用转换和不透明度。  <!--See [Simplify paint complexity and reduce paint areas][WebFundamentalsPerformanceRenderingSimplifyPaintComplexity].  -->  |  
| 影响动画的层分解。  | 过多元素的过度分配 `translateZ(0)` 会大大影响动画性能。  | 尽量少地提升为层，并且仅在你知道它提供切实改进时。  <!--See [Stick to composite-only properties and manage layer count][WebFundamentalsPerformanceRenderingCompositorOnlyProperties].  -->  |  
...
> 原始页面位于此处，[](https://developers.google.com/web/tools/chrome-devtools/rendering-tools/index)作者为[ (Technical][KayceBasques] Writer、Chrome DevTools \& Lighthouse\) 和[Meggin Kearney][MegginKearney] \ (Tech Writer\) 。  
