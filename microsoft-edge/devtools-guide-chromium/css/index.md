...
1.  保留 `Control` \ （Windows \）或 `Command` \ （macOS \），然后选择 " **CSS 示例**" 以在新窗口中打开。  
...
1.  打开 "开发工具"，然后选择左上角的 "**切换设备" 工具栏**图标，或按 `Ctrl` + `Shift` + `M` \ （ `Cmd` + `Shift` + `M` 在 macOS \）。  
...
1.  若要调试媒体查询并在编辑器中打开 CSS 文件， `Sources` 请将鼠标悬停在任何条形图段上，打开上下文菜单 \ （右键单击 \），然后选择 `reveal in source code` 。  
