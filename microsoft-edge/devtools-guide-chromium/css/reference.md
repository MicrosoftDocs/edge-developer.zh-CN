...
以下有关查看和更改 CSS 的 Microsoft Edge DevTools 功能的综合参考中发现新工作流。  

若要了解基础知识，请导航到查看 [和更改 CSS 入门][DevToolsCSSGetStarted]。  

## <a name="choose-an-element"></a>选择元素  

DevTools 的 Elements 工具允许你一次查看或更改一个元素的 CSS。 ****  选定元素在 **DOM 树中突出显示**。  元素的样式显示在" **样式"窗格中** 。  对于教程，导航到[查看元素 的 CSS。][DevToolsCSSGetStartedTutorial]  

> [!NOTE]
> 在下图中，DOM 树中突出显示 `h1` **的元素** 是所选元素。  在右侧，元素的样式显示在" **样式"窗格中** 。  在左侧，该元素在视口中突出显示，但仅仅是因为鼠标当前正悬停在**DOM 树 中。**  
...
*   在视口中，将鼠标悬停在 元素上，打开上下文菜单 \ (右键单击\) ，然后选择"检查 **"。**  
*   在 DevTools 中，选择"选择元素**\ (** 选择元素 ![ \) "或选择 ](../media/select-an-element-icon.msft.png) `Control` + `Shift` + `C` \ (Windows、Linux\) 或 `Command` + `Shift` + `C` \ (macOS\) ，然后选择视口中的元素。  
*   在 DevTools 中，选择 **DOM**树中的 元素。  
...
如果样式表已缩小，请导航到"[使缩小文件可读"。][DevToolsJavascriptReferenceFormat]  

> [!NOTE]
> 在下图中，选择后，您将进入 定义 CSS 规则的第 `https://docs.microsoft.com/_themes/docs.theme/master/en-us/_themes/styles/b66bc881.site-ltr.css:2` 2 `https://docs.microsoft.com/_themes/docs.theme/master/en-us/_themes/styles/b66bc881.site-ltr.css` `.content h1:first-of-type` 行。  
...
" **样式** "面板显示应用于元素的所有规则，包括已替代的声明。  如果对重写声明不感兴趣，请使用 **Computed** 面板仅查看实际应用于元素的 CSS。  

1.  [选择元素](#choose-an-element)。  
1.  导航到 **"元素"工具** 中的"计算 **"** 面板。  

> [!NOTE]
> 在宽的 DevTools 窗口中****，计算面板不存在。  "计算" **面板的内容** 显示在" **样式"面板** 上。  

继承的属性是不透明的。  若要显示所有继承的值，请选中" **全部显示"** 复选框。  

> [!NOTE]
> 下图中，" **计算** "面板显示应用于当前选定元素的 CSS `h1` 属性。  
...
使用 **"计算"** 面板。  导航[到"仅查看实际应用于元素的 CSS"。](#view-only-the-css-that-is-actually-applied-to-an-element)  
...
若要查看 [元素的框][MDNBoxModel] 模型，请导航到" **样式"** 面板。  如果你的 DevTools 窗口较窄，则 **框** 模型图位于面板的底部。  

选择并编辑值以更改值。  

> [!NOTE]
> 在下图中 **，"样式** "面板中的"方框模型 **"图显示** 当前所选元素的框 `h1` 模型。  
...
若要同时在计算面板 **中搜索继承** 的属性，请选中" **全部显示"** 复选框。  

> [!NOTE]
> 在下图中， **将筛选"样式** "面板，以只显示包含搜索查询 的规则 `color` 。  
...
> [!NOTE]
> 在下图中， **将筛选计算** 面板以只显示包含搜索查询 的声明 `100%` 。  
...
> [!NOTE]
> 在下图中，切换 `:hover` 伪类。  在视口中，验证声明是否应用于元素，即使该元素实际上并未悬停在 `background-color: cornflowerblue` 上方。  
...
1.  选择 `Control` + `Shift` + `P` \ (Windows、Linux\) 或 `Command` + `Shift` + `P` \ (macOS\) 而 DevTools 位于[][DevToolsCommandMenu]焦点中以打开命令菜单。  
1.  开始键入并选择 `coverage` "显示**覆盖"。**  将显示 **"覆盖** "工具。  
...
1.  选择 **开始检测覆盖范围并刷新** 页面 \ (![ 开始检测范围并刷新 ](../media/refresh-icon.msft.png) 页面 \) 。  页面刷新和覆盖工具概述了**** 浏览器加载的每个文件使用多少 CSS \ (和 JavaScript\) 。  绿色表示使用的 CSS。  红色表示未使用的 CSS。  
...
1.  若要显示 CSS 使用的明细数据，请选择 CSS 文件。  
    
    > [!NOTE]
    > 在下图中，未使用第 145 至 147 行以及 149 到 151 行，而 `b66bc881.site-ltr.css` 使用第 163 至 166 行。  
...
声明的顺序会影响元素的样式，请使用以下列表来帮助你以不同方式添加声明。  

*   [添加内嵌声明](#add-an-inline-declaration)。  相当于将 `style` 属性添加到元素的 HTML。  
*   [向样式规则添加声明](#add-a-declaration-to-a-style-rule)。  

**您应该使用什么工作流？** 对于大多数方案，您可能想要使用内嵌声明工作流。  内嵌声明比外部声明具有更高的特定性，因此内联工作流可确保更改在预期元素中生效。  有关特定性详细信息，请导航到选择 [器类型][MDNSelectorTypes]。  

如果要调试元素的任何样式，并且需要专门测试在不同位置定义声明时会发生什么情况，请使用其他工作流。  

#### <a name="add-an-inline-declaration"></a>添加内嵌声明  
...
1.  输入属性名称并选择 `Enter` 。  
1.  输入该属性的有效值，然后选择 `Enter` 。  在 **DOM 树**中，验证 `style` 属性已添加到 元素。  

> [!NOTE]
> 在下图中，和 `margin-top` `background-color` 属性已应用于选定元素。  在 **DOM 树** 中，验证声明是否反映在 `style` 元素的 属性中。  
...
选择并编辑声明的名称或值以对其进行更改。  对于通过、或单位快速递增或缩小值的快捷方式，请导航到"使用键盘快捷方式 `0.1` `1` `10` `100` [更改声明值"。](#change-declaration-values-with-keyboard-shortcuts)  
...
编辑声明的值时，可以使用以下键盘快捷方式将值增加一定量。  

*   选择 `Alt` + `Up` \ (Windows、Linux\) 或 `Option` + `Up` \ (macOS\) 递增 `0.1` 。  
*   选择 将值更改为 ，如果当前值介于 和 之间，则按 `Up` `1` `0.1` `-1` 更改 `1` 值。  
*   选择 `Shift` + `Up` 以按 递增 `10` 。  
*   选择 `Shift` + `Page Up` \ (Windows、Linux\) 或 `Shift` + `Command` + `Up` \ (macOS\) `100` 将值递增。  

减减也有效。  只需将上述每个 `Up` 实例替换为 `Down` 。  

### <a name="add-a-class-to-an-element"></a>向元素添加类  

完成以下操作以向元素中添加类。  

1.  [选择 DOM](#choose-an-element) 树 **中的 元素**。  
1.  选择 **".cls"。**  
1.  在"添加新类"文本框 **中输入类** 的名称。  
1.  选择 `Enter`。  
...
1.  [选择 DOM](#choose-an-element) 树 **中的 元素**。  
1.  打开 **"元素类"** 窗格。  导航到["将类添加到元素"。](#add-a-class-to-an-element)  在" **添加新类** "文本框下方是应用于特定元素的所有类。  
1.  切换要打开或关闭的类旁边的复选框。  
...
1.  [选择元素](#choose-an-element)。  
1.  Choose **New Style Rule** \ (New Style Rule ![ ](../media/new-style-rule-icon.msft.png) \) .  DevTools 在 **element.style 规则下插入一个新** 规则。  

> [!NOTE]
> 在下图中，DevTools 在选择"新建样式规则"后 `h1.devsite-page-title` **添加样式规则**。  
...
添加新[样式规则](#add-a-style-rule)时，选择并按住"新建**** 样式规则" ("新建样式规则) 以选择要向哪个样式 ![ ](../media/new-style-rule-icon.msft.png) 表添加样式规则。  
...
*   在重点样式规则正下方插入样式规则。  
*   将 `background-color` 、 `color` `box-shadow` 、 或 `text-shadow` 声明添加到你侧重于的样式规则。  

完成以下操作以显示"更多 **操作"** 工具栏。  

1.  在" **样式"** 面板中，将鼠标悬停在样式规则上。  **更多操作** \ (\) 在样式规则部分右下 `...` 角显示。  
    
    > [!NOTE]
    > 在下图中，将鼠标悬停在样式规则上，"更多操作"显示于样式规则节的 `.header-holder.has-default-focus` 右下角。 ****  
...
1.  将鼠标 **悬停在"更多** 操作 (`...` \) 可显示上述操作。  
    
    > [!NOTE]
    > 将 **鼠标悬停在"更多** 操作"上后，显示"插入样式规则下方 **"操作**。  
...
完成创建操作以在 \ (或 off\) 上切换单个) 。  

1.  [选择元素](#choose-an-element)。  
1.  在 **"样式** "窗格中，将鼠标悬停在定义声明的规则上。  每个声明旁边都显示一个复选框。  
1.  选中\ (或取消选中\) 声明旁边的复选框。  取消选中声明时，DevTools 会交叉它以指示它不再处于活动状态。  

> [!NOTE]
> 在下图中， `margin-top` 当前所选元素的属性已切换为关闭。  
...
完成以下操作以向 `background-color` 元素添加声明。  
...
1.  [选择元素](#choose-an-element)。  
1.  在 **"样式** "面板中，查找要更改的 、 或类似 `color` `background-color` 声明。  在 、 或类似值的左侧，有一个小 `color` `background-color` 正方形，它是颜色预览。  
    
    > [!NOTE]
    > 在下图中，左侧的小正方形 `rgba(0, 0, 0, 0.7)` 是该颜色的预览。  
...
下图和列表对颜色选取器的每个 UI 元素的**descries。**  
...
:::row:::
   ...
   :::column-end:::
   :::column span="2":::
      有关详细信息，请导航到使用目 [视器从页面上采样颜色](#sample-a-color-off-the-page-with-the-eyedropper)。  
   :::column-end:::
:::row-end:::  
:::row:::
   :::column span="1":::
...
:::row:::
   ...
   :::column-end:::
   :::column span="2":::
      在当前颜色的 RGBA、HSLA 和 Hex 表示法之间进行切换。  
   :::column-end:::
:::row-end:::  
:::row:::
   :::column span="1":::
   ...
   :::column-end:::
   :::column span="2":::
      在"材料 [设计"调色板][MaterialDesignColorSystem]、自定义调色板或页面颜色调色板之间进行切换。  DevTools 根据它在样式表内找到的颜色生成页面调色板。  
   :::column-end:::
:::row-end:::  

#### <a name="sample-a-color-off-the-page-with-the-eyedropper"></a>使用目视器为页面采样颜色  

打开颜色 **选取器**时，取色器 **\ (** 取色器 ![ ](../media/eyedropper-icon.msft.png) \) 默认打开。  完成以下操作，将所选颜色更改为页面上的某种其他颜色。  

1.  将鼠标悬停在视口中的目标颜色上。  
1.  选择确认。  
    
    > [!NOTE]
    > 在下图中，颜色 **选取** 器显示 的当前颜色值 `rgba(0,0,0,0.7)` 接近黑色。  选择后，特定颜色应更改为当前在视口中突出显示的黑色版本。  
