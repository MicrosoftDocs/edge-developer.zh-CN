...
导航到 [分析运行时性能][DevtoolsEvaluatePerformanceGettingStarted] 以了解如何使用 [Microsoft Edge DevTools][MicrosoftEdgeDevTools]分析页面性能的引导式教程。  
...
1.  选择 " **刷新页面** \ (![ 刷新" 页面 ][ImageRefreshPageIcon] \ ) 。  DevTools 在页面刷新时记录性能指标，然后在加载完成后的几秒钟内自动停止录制。  
...
导航到 " [查看屏幕截图](#view-a-screenshot) " 以了解如何与屏幕截图交互。  

### 录制时强制垃圾回收  

录制页面时，请选择 " **收集垃圾** \ (![ 收集垃圾图标 ][ImageCollectGarbageIcon] \ ) " 以强制垃圾回收。  
...
选择 " **捕获设置** \ (![ 捕获设置 ][ImageCaptureSettingsIcon] \ ) " 以公开有关 DevTools 如何捕获性能录制的更多设置。  
...
若要了解如何与画图信息交互，请导航到 " [视图图层](#view-layers-information) " 和 " [查看画图" 探查器](#view-paint-profiler)。  
...
录制完毕后，请按 " **清除录制** \ (![ 清除录制" 图标 ][ImageClearRecordingIcon] \ ) 清除 " **性能** " 面板中的录制。  
...
选择 `Control` + `F` \ (Windows、Linux \ ) 或 `Command` + `F` \ (macOS \ ) 以打开 "**性能**" 面板底部的 "搜索" 框。  
...
*   选择 `Shift` + `Enter` 以选择 "上一个" 或 `Enter` 选择下一个。  
...
如果想要隐藏 JavaScript 调用的详细火焰图表，请导航到 " [禁用 javascript 示例](#disable-javascript-samples) "。  如果禁用了 JS 示例，则只能查看 `Event: click` `Function Call` 上图中的高级事件，例如和。  
...
*   当您想要查看最常被直接花费的时间的活动时，请使用 ["Bottom-Up" 选项卡](#the-bottom-up-tab)。  
...
导航到 ["调用树" 选项卡](#the-call-tree-tab) ，查看根活动的示例。  

#### "调用树" 选项卡  

使用 " **调用树** " 选项卡查看哪些 [根活动](#root-activities) 导致最大的工作。  

" **调用树** " 选项卡仅在录制的选定部分中显示活动。  导航以 [选择录制部分](#select-a-portion-of-a-recording) ，了解如何选择部分内容。  
...
选择 " **自时间**"、" **总时间**" 或 " **活动** "，按该列对表格进行排序。  

使用 " **筛选** " 文本框按活动名称筛选事件。  

默认情况下，" **分组** " 菜单设置为 " **无分组**"。  使用 " **分组** " 菜单基于各种条件对活动表进行排序。  

选择 "显示最大的 **堆栈** \ (显示最大 ![ ][ImageShowHeaviestStackIcon] 的堆栈 \" ) ，将另一个表显示在 **活动** 表的右侧。  单击活动以填充最大的 **堆栈** 表。  "最 **繁忙的堆栈** " 表显示所选活动的哪些子元素运行时间最长。  

#### "Bottom-Up" 选项卡  

使用 " **下** " 选项卡查看哪些活动直接占用总时间。  

**自下而上**的选项卡仅显示录制的选定部分中的活动。  导航以 [选择录制部分](#select-a-portion-of-a-recording) ，了解如何选择部分内容。  
...
**在上图的第一**节火焰图表中，导航到几乎几乎几乎所有时间都在运行 `Parse HTML` 。  上图的 " **下** 一步" 选项卡中的顶部活动是 `Parse HTML` 。  <!--In the flame chart of the previous figure, the yellow below the calls to `wait` are actually thousands of `Minor GC` calls.  -->  导航到 " **下** 一步" 选项卡，下一个开销最高的活动是 `Layout` 。  
...
" **事件日志** " 选项卡仅显示录制的选定部分中的活动。  导航以 [选择录制部分](#select-a-portion-of-a-recording) ，了解如何选择部分内容。  
...
*   当页面运行时，使用 **fps 计量** 器以实时估计 fps。  导航到 ["每秒查看帧数" 和 "FPS" 计量](#view-frames-per-second-in-realtime-with-the-fps-meter)器。  
...
[在录制时导航到 "捕获屏幕截图](#capture-screenshots-while-recording)"，了解如何启用屏幕截图。  
...
*   选择 " **平移模式** \ (![ 平移模式 ][ImagePanModeIcon] \ ) " 沿 X 轴和 Y 轴移动。  
*   选择 " **旋转模式** \ (![ 旋转模式 \ ) " ][ImageRotateModeIcon] 沿 Z 轴旋转。  
*   选择 " **重置转换** \ (![ 重置转换 \" ][ImageResetTransformIcon] ) 将图表重置为原始位置。  
...
1.  打开 " **呈现** " 选项卡。 Na [通过 "渲染" 选项卡分析渲染性能](#analyze-rendering-performance-with-the-rendering-tab)。  
1.  启用 " **FPS 指示器** " 复选框。  
...
1.  打开 " **呈现** " 选项卡。 导航到 ["呈现" 选项卡上的 "分析渲染性能"](#analyze-rendering-performance-with-the-rendering-tab)。  
1.  启用 " **绘画闪烁** " 复选框。  
...
导航到 [debug_colors][ChromiumDebugColors] 中的批注，查看 codings 的说明。  
