...
当然，应看到页面性能不佳。  但是在实际场景中可能就不是那么清楚了，所以掌握所有的工具进行测量就会方便很多。  
...
1.  禁用 **FPS 计数器**，然后选择 `Escape` 以关闭 “**渲染**”选项卡。本教程中未使用 **FPS计数器**。  
...
1.  记录中有很多数据。  如果要放大单个事件；请选择、按住并将光标拖动到**Overview**上，即包括**FPS**、**CPU**和**NET**图表的部分。  "**重点**" 部分和 "**摘要**" 选项卡仅显示录制所选部分的信息。  
    ...
    1.  重点关注 **动画帧触发**事件右上角的红色三角形。  每当看到一个红色的三角形，则是一个表示可能有与事件有关问题的警告。  
    
    > [!NOTE]
    > 每当运行[ `requestAnimationFrame()`回调][MDNWebRequestAnimationFrame]时，就会发生**动画帧触发**事件。  
    
1.  选择 **动画帧触发**事件。  "**摘要**" 选项卡现在向你显示有关该事件的信息。  请注意 "**显示**" 链接。  选择后，DevTools 将突出显示启动 **动画帧触发** 事件。  另外，请关注**app.js:95**链接。  选择后，将显示源代码中的相关行。
...
1.  在“**摘要**”选项卡中，选择**强制布局**“下的**app.js:71**链接。  DevTools 将转到强制布局的代码行。  
    ...
    > [!NOTE]
    > 这段代码的问题在于，在每个动画帧中，它都会改变每个图标的样式，然后查询每个图标在页面上的位置。  由于样式发生变化，而浏览器不知道每个图标的位置是否发生变化，所以要重新布局图标，才能计算出新的位置。  <!--  > See [Avoid forced synchronous layouts][RenderingAvoidSynchronousLayouts] to learn more.  -->
...
利用刚才学习的工作流程和工具，在演示中选择**优化**以启用优化后的代码，再进行一次性能记录，然后分析结果。  从帧率的提升到**主要**部分火焰图表中事件的减少，能够看到优化版的应用所做的工作更少，从而获得更好的性能。  

> [!NOTE]
> 即使优化版本也不很好，因为它可以操纵每个图标的`top`属性。  更好的方法是保留仅影响合成的属性。  <!--  > See [Use transform and opacity changes for animations][RenderingCompositor] for more information.  -->  
...
若要更轻松地使用 "性能" 面板，请多多操练，所谓熟能生巧。  试着对页面进行剖析并分析结果。  如果对结果有任何疑问，请使用**发送反馈** 图标，选择 `Alt`+`Shift`+`I`  \(Windows, Linux\)，选择 `Option`+`Shift`+`I`  \(macOS\)， 或[发 tweet 给 DevTools 团队][TwitterEdgeDevtools]。  如果可能，请包括屏幕截图或指向可重现页面的链接。  
