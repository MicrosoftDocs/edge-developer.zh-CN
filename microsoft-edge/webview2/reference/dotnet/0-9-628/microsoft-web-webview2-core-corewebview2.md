...
[ContainsFullScreenElementChanged](#containsfullscreenelementchanged) | 当 ContainsFullScreenElement 属性更改时，将触发 ContainsFullScreenElementChanged。
[ContentLoading](#contentloading) | ContentLoading 在加载任何内容之前激发，包括通过 AddScriptToExecuteOnDocumentCreated 添加的脚本。
[DocumentTitle](#documenttitle) | 当前顶级文档的标题。
[DocumentTitleChanged](#documenttitlechanged) | 当 Web 视图的 DocumentTitle 属性发生更改，并且可能会在 NavigationCompleted 事件之前或之后出现时，DocumentTitleChanged 会引发此事件。
[FrameNavigationCompleted](#framenavigationcompleted) | 当子框架已完全加载 (正文中时，FrameNavigationCompleted 引发) 或加载已停止，但出现错误。
[FrameNavigationStarting](#framenavigationstarting) | 当 Web 视图中的子框架请求导航到其他 URI 的权限时，将触发 FrameNavigationStarting。
[HistoryChanged](#historychanged) | 历史记录更改侦听顶级文档的导航历史记录更改。
[NavigationCompleted](#navigationcompleted) | 当 Web 视图已完全加载 (正文时，NavigationCompleted 激发) 或加载时已停止，但出现错误。
[NavigationStarting](#navigationstarting) | 当 Web 视图主框架请求导航到其他 URI 的权限时，将触发 NavigationStarting。
[Webview.newwindowrequested](#newwindowrequested) | 当 Web 视图中的内容请求打开新窗口（如通过 window）时，将触发 Webview.newwindowrequested。
[Webview.permissionrequested](#permissionrequested) | 当 Web 视图中的内容请求访问某些特权资源的权限时，将触发 Webview.permissionrequested。
[ProcessFailed](#processfailed) | 当 Web 视图进程意外终止或变得不响应时，ProcessFailed 将激发。
[ScriptDialogOpening](#scriptdialogopening) | 在将显示 Web 视图的 JavaScript 对话框 (警报、确认、提示或 beforeunload) 时，将引发此事件。
...
[WebMessageReceived](#webmessagereceived) | 当设置 CoreWebView2Settings IsWebMessageEnabled 设置和 Web 视图调用的顶级文档时，将触发 WebMessageReceived `window.chrome.webview.postMessage` 。
[WebResourceRequested](#webresourcerequested) | 当 URL 请求通过网络 (、文件等 ) 在 Web 资源匹配的 Web 资源和 AddWebResourceRequestedFilter 中指定的 URL 的 Web 视图中进行时，将触发 WebResourceRequested。
...
[WindowCloseRequested](#windowcloserequested) | 当 Web 视图中的内容请求关闭窗口时（例如，在窗口后关闭窗口）时，将触发 WindowCloseRequested。调用 close。
...
当 ContainsFullScreenElement 属性更改时，将触发 ContainsFullScreenElementChanged。
...
ContentLoading 在加载任何内容之前激发，包括通过 AddScriptToExecuteOnDocumentCreated 添加的脚本。
...
如果发生相同的页面导航 (例如通过片段导航或 pushState 导航) ，将不会触发 ContentLoading。
...
当 Web 视图的 DocumentTitle 属性发生更改，并且可能会在 NavigationCompleted 事件之前或之后出现时，DocumentTitleChanged 会引发此事件。

> 公共事件 EventHandler< 对象 > [DocumentTitleChanged](#documenttitlechanged)

#### FrameNavigationCompleted 

当子框架已完全加载 (正文中时，FrameNavigationCompleted 引发) 或加载已停止，但出现错误。
...
当 Web 视图中的子框架请求导航到其他 URI 的权限时，将触发 FrameNavigationStarting。
...
这也会引发重定向。 在事件处理程序返回之前，可以阻止相应的导航。
...
使用历史记录更改检查 CanGoBack/CanGoForward 值是否已更改。 使用 GoBack/GoForward 时也会触发 HistoryChanged。 HistoryChanged 在 SourceChanged 和 ContentLoading 后激发。

#### NavigationCompleted 

当 Web 视图已完全加载 (正文时，NavigationCompleted 激发) 或加载时已停止，但出现错误。
...
当 Web 视图中的内容请求打开新窗口（如通过 window）时，将触发 Webview.newwindowrequested。
...
应用可以传递将被视为打开的窗口的目标 Web 视图。 如果未在事件参数上获取延迟，则请求的新窗口可能会被阻止，直到事件处理程序返回。 如果采取延迟，将阻止脚本，直到完成延期。

#### Webview.permissionrequested 

当 Web 视图中的内容请求访问某些特权资源的权限时，将触发 Webview.permissionrequested。
...
如果未对事件参数执行延迟，则可以阻止后续脚本，直到事件处理程序返回。 如果采取延迟，将阻止脚本，直到完成延期。

#### ProcessFailed 

当 Web 视图进程意外终止或变得不响应时，ProcessFailed 将激发。
...
在将显示 Web 视图的 JavaScript 对话框 (警报、确认、提示或 beforeunload) 时，将引发此事件。
...
仅当 AreDefaultScriptDialogsEnabled 属性设置为 false 时，此事件才会触发 CoreWebView2Settings。 ScriptDialogOpening 事件可用于取消对话框或使用自定义对话框替换默认对话框。 如果未对事件参数执行延迟，则可以阻止后续脚本，直到事件处理程序返回。 如果采取延迟，将阻止脚本，直到完成延期。
...
导航到其他网站或片段导航时，将引发 SourceChanged。 不会为其他类型的导航（如页面重新加载或 pushState 与当前页面相同的 URL）触发。 SourceChanged 将在 ContentLoading 之前激发，以便导航到新文档。

#### WebMessageReceived 

当设置 CoreWebView2Settings IsWebMessageEnabled 设置和 Web 视图调用的顶级文档时，将触发 WebMessageReceived `window.chrome.webview.postMessage` 。
...
PostMessage 函数是 `void postMessage(object)` 对象是 JSON 转换支持的任何对象。 调用 postMessage 时，将使用转换为 JSON 字符串的 postMessage 对象参数调用处理程序的 Invoke 方法。

#### WebResourceRequested 

当 URL 请求通过网络 (、文件等 ) 在 Web 资源匹配的 Web 资源和 AddWebResourceRequestedFilter 中指定的 URL 的 Web 视图中进行时，将触发 WebResourceRequested。
...
宿主可以查看和修改请求，或向 HTTP 提供类似模式的响应，在这种情况下，请求立即完成。 这可能不包含由网络堆栈添加的任何请求标头，如授权标头。 请求的 web 资源可以在事件处理程序返回之前被阻止，直到未对事件参数执行延迟。 如果获取延迟，则将阻止请求的 web 资源，直到延期完成。
...
当 Web 视图中的内容请求关闭窗口时（例如，在窗口后关闭窗口）时，将触发 WindowCloseRequested。调用 close。
...
若要在 c # 中创建 IDispatch 实现类，请在要公开的每个类上使用以下属性。 
```
...
```
 然后通过以下方式添加这些类的实例 `AddHostObjectToScript` ： 
```
webView.CoreWebView2.AddHostObjectToScript("bridge", new Bridge());
```
 然后在脚本中，您可以调用这些方法，并通过 AddHostObjectToScript 访问这些对象的属性： 
```
...
HtmlContent 参数的总大小不得大于 2 MB。 新页面的来源将显示为 "空白"。
...
事件参数是的实例 `MessageEvent` 。 CoreWebView2Settings IsWebMessageEnabled 设置必须为 true，否则此方法将失败，并 E_INVALIDARG。 事件参数的数据属性是将其作为 JSON 字符串分析到 JavaScript 对象中的 webMessage 字符串参数。 事件 arg 的 source 属性是对该对象的引用 `window.chrome.webview` 。 有关将消息从 Web 视图中的 HTML 文档发送到主机的信息，请参阅 WebMessageReceived 事件。 此消息将异步发送。 如果在将邮件发布到页面之前发生导航，则不会发送邮件。
