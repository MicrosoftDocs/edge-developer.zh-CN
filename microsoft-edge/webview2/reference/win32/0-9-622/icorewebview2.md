...
## 摘要
 ...
 成员                        | 描述
--------------------------------|---------------------------------------------
[add_ContainsFullScreenElementChanged](#add_containsfullscreenelementchanged) | 为 ContainsFullScreenElementChanged 事件添加事件处理程序。
...
[add_HistoryChanged](#add_historychanged) | 为 HistoryChanged 事件添加事件处理程序。
...
[add_SourceChanged](#add_sourcechanged) | 为 SourceChanged 事件添加事件处理程序。
[add_WebMessageReceived](#add_webmessagereceived) | 为 WebMessageReceived 事件添加事件处理程序。
...
[重载](#reload) | 重新加载当前页面。
[remove_ContainsFullScreenElementChanged](#remove_containsfullscreenelementchanged) | 删除以前使用 add_ContainsFullScreenElementChanged 添加的事件处理程序。
...
为 ContainsFullScreenElementChanged 事件添加事件处理程序。
...
当 ContainsFullScreenElement 属性更改时，将触发 ContainsFullScreenElementChanged。 这意味着 Web 视图中的 HTML 元素正在将全屏输入到 Web 视图的大小或离开全屏。 例如，当视频元素请求进入全屏时，此事件非常有用。 然后，ContainsFullScreenElementChanged 的侦听器可以在响应中调整 Web 视图的大小。
...
ContentLoading 在加载任何内容之前激发，包括通过 AddScriptToExecuteOnDocumentCreated 添加的脚本。 如果发生相同的页面导航 (例如通过片段导航或 pushState 导航) ，将不会触发 ContentLoading。 这将遵循 NavigationStarting 和 SourceChanged 事件，并在 HistoryChanged 和 NavigationCompleted 事件之前。
...
当 Web 视图中的子框架请求导航到其他 URI 的权限时，将触发 FrameNavigationStarting。 这也会引发重定向。

在事件处理程序返回之前，可以阻止相应的导航。
...
HistoryChanged 侦听顶级文档的导航历史记录更改。 使用 HistoryChanged 检查 CanGoBack/CanGoForward 值是否已更改。 使用 GoBack/GoForward 时也会触发 HistoryChanged。 HistoryChanged 在 SourceChanged 和 ContentLoading 后激发。
...
当 Web 视图中的内容请求打开新窗口（如通过 window）时，将触发 Webview.newwindowrequested。 应用可以传递将被视为打开的窗口的目标 Web 视图。

如果未在事件参数上获取延迟，则请求的新窗口可能会被阻止，直到事件处理程序返回。 如果采取延迟，将阻止脚本，直到完成延期。
...
当将显示 web 视图的 JavaScript 对话框 (警报、确认、提示或 beforeunload) 时，将引发 ScriptDialogOpening。 仅当 ICoreWebView2Settings：： AreDefaultScriptDialogsEnabled 属性设置为 false 时，此事件才会触发。 ScriptDialogOpening 事件可用于取消对话框或使用自定义对话框替换默认对话框。
...
Source 属性更改时将触发 SourceChanged。 导航到其他网站或片段导航时，将引发 SourceChanged。 不会为其他类型的导航（如页面重新加载或 pushState 与当前页面相同的 URL）触发。 SourceChanged 将在 ContentLoading 之前激发，以便导航到新文档。
...
为 WebMessageReceived 事件添加事件处理程序。
...
当设置 ICoreWebView2Settings：： IsWebMessageEnabled 设置和 Web 视图调用的顶级文档时，将触发 WebMessageReceived `window.chrome.webview.postMessage` 。 PostMessage 函数是 `void postMessage(object)` 对象是 JSON 转换支持的任何对象。
...
```
 调用 postMessage 时，将使用转换为 JSON 字符串的 postMessage 对象参数调用处理程序的 Invoke 方法。
...
当 Web 视图正在对使用 AddWebResourceRequestedFilter 添加的匹配 URL 和资源上下文筛选器执行 URL 请求时，将触发 WebResourceRequested。 必须至少添加一个筛选器，才能触发该事件。

请求的 web 资源可以在事件处理程序返回之前被阻止，直到未对事件参数执行延迟。 如果获取延迟，则将阻止请求的 web 资源，直到延期完成。
...
当 Web 视图中的内容请求关闭窗口时（例如，在窗口后关闭窗口）时，将触发 WindowCloseRequested。调用 close。 如果应用程序有意义，则应用应关闭 Web 视图和相关应用窗口。
...
这将异步执行，并且在完成后，如果 ExecuteScriptCompletedHandler 参数中提供了处理程序，则将通过评估所提供的 JavaScript 的结果调用其 Invoke 方法。 结果值是一个 JSON 编码的字符串。 如果结果未定义、包含引用循环或者其他无法编码到 JSON，则将以字符串 "null" 形式返回 JSON null 值。 请注意，没有显式返回值的函数将返回 undefined。 如果执行的脚本引发了未处理的异常，则结果也为 "null"。 此方法是异步应用的。 如果在导航过程中 NavigationStarting 事件后调用该方法，则会在加载新文档时在新文档中执行该脚本，同时引发 ContentLoading。 即使 ICoreWebView2Settings：： IsScriptEnabled 设置为 FALSE，ExecuteScript 也可以正常工作。
...
删除以前使用 add_ContainsFullScreenElementChanged 添加的事件处理程序。
