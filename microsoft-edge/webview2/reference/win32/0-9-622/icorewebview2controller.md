...
在窗口模式下，此事件处理程序是同步调用的。 在调用已处理 ( 事件参数或事件处理程序上的 # A1 之前，浏览器进程将被阻止，并且传出的进程间 COM 调用将失败，并出现 RPC_E_CANTCALLOUT_ININPUTSYNCCALL。 但是，所有 CoreWebView2 API 方法都有效。

在无窗口模式下，异步调用事件处理程序。 在事件处理程序返回或处理之前，其他输入将无法访问浏览器 ( # A1 被调用，但浏览器进程本身将不会被阻止，并且传出 COM 调用将正常工作。

建议调用 (TRUE) 处理，因为你知道要处理加速键。
...
当 Web 视图失去焦点时，将激发 LostFocus。 在引发 MoveFocusRequested 事件的情况下，当 MoveFocusRequested 事件引发时，焦点仍在 Web 视图上。 仅当应用的 MoveFocusRequested 事件的代码或默认操作设置焦点离开 Web 视图时，LostFocus 才会随后引发。
...
当 Web 视图的 ZoomFactor 属性更改时，将触发 ZoomFactorChanged。 由于调用方修改了 ZoomFactor 属性，或者由于用户手动修改缩放，因此可能会激发该事件。 当调用方通过 ZoomFactor 属性对其进行修改时，内部缩放系数将立即更新，并且将不会出现 ZoomFactorChanged 事件。 Web 视图将每个网站的上次使用的缩放系数相关联。 因此，在导航到其他页面时，可以更改缩放系数。 当缩放系数因此而更改时，ZoomFactorChanged 事件将在 ContentLoading 事件后立即触发。
...
> 公共 HRESULT [get_ParentWindow](#get_parentwindow) (HWND * ParentWindow) 
...
> 公共 HRESULT [put_ParentWindow](#put_parentwindow) (HWND ParentWindow) 
