...
1.  在 "解决方案资源管理器" 中，生成两个项目。  
    ...
    *   **项目名称 (程序包) **。  此项目是 aWindows 应用程序打包 Projectthat 配置为将应用构建到 MSIX 程序包中以进行部署。  该项目包含 thepackage manifestfor 你的应用，并且默认情况下它是你的解决方案的启动项目。 有关详细信息，请参阅 [在 Visual Studio 中设置用于 MSIX 打包的桌面应用程序][WindowsMsixDesktopToUwpPackagingDotNet] 和 [适用于 Windows 10 的程序包清单架构参考][UwpSchemasAppxpackageUapmanifestRoot]。
