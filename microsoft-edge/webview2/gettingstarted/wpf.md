...
1. 打开**Visual Studio。**
2. 选择 " **wpf .Net Core app** " 或 " **Wpf .net Framework 应用**"，然后选择 "**下一步**"。  
...
2. 通过 `<Grid>` 使用以下代码片段替换标记来添加 WebView2 控件。  该 `Source` 属性设置 WebView2 控件中显示的初始 URI。  
...
3. 按 `F5` 生成并运行你的项目。  确认你的 WebView2 控件是否显示 [https://www.microsoft.com](https://www.microsoft.com) 。  
...
2. `MainWindow.xaml.cs`在 Visual Studio 中打开。  `CoreWebView2`通过在的顶部插入以下代码片段来添加命名空间 `MainWindow.xaml.cs` 。  

    ```csharp
    using Microsoft.Web.WebView2.Core;
    ```

3. 在**MainWindow.xaml.cs**中，复制以下代码片段以创建 `ButtonGo_Click` 方法，该方法将 web 视图导航到在地址栏中输入的 URL。  
...
按 `F5` 生成并运行你的项目。  在地址栏中输入新的 URL，然后单击 "**转到**"。  例如，enter `https://www.bing.com` 。  确认 WebView2 控件导航到 URL。  

> [!NOTE]
> 请确保在地址栏中输入完整的 URL。 `ArgumentException`如果 URL 不以 or 开头的 URL，则会引发 `http://` `https://`
...
按 `F5` 生成并运行你的项目。 确认在导航到 HTTP 站点时，Web 视图**保持不变**。 但是，Web 视图将导航到 HTTPS 站点。
...
2. 初始化**CoreWebView2**后，注册一个事件处理程序以响应 `WebMessageReceived` 。  在**MainWindow.xaml.cs**更新中 `InitializeAsync` ，并 `UpdateAddressBar` 使用以下代码片段添加。  
...
3. 为了让 web 视图发送和响应 web 消息，在 `CoreWebView2` 初始化后，主机：  

    1. 将脚本插入到 web 内容中，该内容注册处理程序以打印来自主机的消息。  
    2. 将脚本插入到将 URL 发布到主机的 web 内容中。  
...
* 签出[api 参考](../reference/wpf/0-9-515/microsoft-web-webview2-wpf-webview2.md)了解有关我们的 api 的更多详细信息
