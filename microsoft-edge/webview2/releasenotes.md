...
WebView2 团队会以六周节奏更新[WebView2 SDK。][NuGetGallery]  查看以下内容，获取有关产品公告、添加、修改和 API 的中断更改最新信息。  

> [!NOTE]
> 在更新 NuGet 程序包后重新编译应用。  

## 1.0.721-prerelease  

发布日期：2020 年 12 月 8 日  

[NuGet 包][NuGetGallery1.0.721-prerelease] \|最低 Microsoft Edge 版本 86.0.616.0。  
...
*   添加了对 [WebView2 &拖放][ReferenceWin32Icorewebview2experimentalcompositioncontroller3] 的支持。  
*   添加了 API 以处理 DPI 支持。  
    ...
    *   添加了 [BoundsMode][ReferenceWin32Icorewebview2experimentalcontrollerViewWebview210721PrereleaseGetBoundsMode] 属性，以指定边界是逻辑像素，并允许 WebView 使用 for WebView2 像素显示，并应用 `RasterizationScale` WebView 获取 `RasterizationScale` `Bounds` 物理大小。
...
[NuGet 包][NuGetGallery1.0.664.37] \|最低 WebView2 运行时版本 86.0.616.0。  

#### 常规  

> [!IMPORTANT]
> **公告**：.NET WPF/WinForms WebView2 现已正式发布 \ (GA\) 。  从此版本开始，版本 SDK 是向前兼容的。  有关详细信息，请导航到 [GA 公告博客文章][MicrosoftDevblogDotnetAnnouncingGeneralAvailabilityForMicrosoftEdgeWebview2ForNetFixedDistributionMethod]。  
...
[NuGet 包][NuGetGallery1.0.674-prerelease] \|最低 WebView2 运行时版本 86.0.616.0。  

#### 常规  

*   添加了 [NavigateWithWebResourceRequest][ReferenceWin32Icorewebview2experimentalNavigatewithwebresourcerequest10674] 方法，可让你在导航期间提供发布数据或其他请求标头。
...
*   添加了 [Cookie 管理][ReferenceWin32Icorewebview2experimentalGetCookiemanager10674] API，允许开发人员对 WebView2 会话进行身份验证，或从 WebView 检索 Cookie 以验证其他工具。  我们计划进行特定于语言/框架的改进。  有关详细信息，请导航到["API 审阅：Cookie 管理"。][GithubMicrosoftedgeWebview2AnnouncementIssue2]
*   更新[了 WebResourceResponseReceived][ReferenceWin32Icorewebview2experimentalAddWebresourceresponsereceived10674]事件，并添加了不可变[WebResourceResponseView][ReferenceWin32Icorewebview2experimentalwebresourceresponseview10674]和[WebResourceResponseReceivedEventArgs：:P opulateResponseContent][ReferenceWin32Icorewebview2experimentalwebresourceresponsereceivedeventargsPopulateresponsecontent09628]变为[WebResourceResponseView：：GetContent。][ReferenceWin32Icorewebview2experimentalwebresourceresponseviewGetcontent10674]
...
*   使用 WebView2 静态库时，开发人员不再需要包含 version.lib。

#### .NET  

*   更新 [了 CoreWebView2][DotnetApiMicrosoftWebWebview2CoreCorewebview2] 类以公开 CoreWebView2Environment 变量。  
*   将命名空间中的自定义 EventArgs 类的实现更改为 `Microsoft.Web.WebView2.Core` [System.EventArgs][DotnetApiSystemEventargs] 或 [System.ComponentModel.CancelEventArgs 的子类][DotnetApiSystemComponentmodelCancelEventargs]。  \ ([\#250][GithubMicrosoftedgeWebviewfeedbackIssue250]\)   
...
*   更新 [了 BuildWindowCore][DotnetApiMicrosoftWebWebview2WpfWebview2Buildwindowcore] 和 [DestroyWindowCore，][DotnetApiMicrosoftWebWebview2WpfWebview2Destroywindowcore] 更加可靠。  \ ([\#382][GithubMicrosoftedgeWebviewfeedbackIssue382]\) 。  
...
[NuGet 包][NuGetGallery1.0.622.22] \|最低 WebView2 运行时版本 86.0.616.0。  

#### 常规  

> [!IMPORTANT]
> **公告**：Win32 C/C++ WebView2 现已正式发布 \ (GA\) 。  从此版本开始，版本 SDK 是向前兼容的。  有关详细信息，请导航到 [GA 公告博客文章][WindowsBlogsMsedgedevEdgeWebview2GeneralAvailability]。  

*  [常青 WebView2 运行时和安装程序][Webview2ConceptsDistributionUnderstandRuntimeInstaller] 是 GA。  适用于常青运行时的引导程序、引导程序下行链接和独立安装程序在 [Microsoft Edge WebView2 上可用][MicrosoftDeveloperMicrosoftEdgeWebView2]。  [WebView2Samples][GithubMicrosoftedgeWebview2samplesMain]存储库也提供了安装工作流的示例代码。  
...
[NuGet 包][NuGetGallery0.9.622.11] \|最低 WebView2 运行时版本 86.0.616.0。  
...
*   在 [WebView2 环境选项上添加了 AllowSingleSignOnUsingOSPrimaryAccount][ReferenceWin32Icorewebview2environmentoptionsGetAllowsinglesignonusingosprimaryaccount09622] 属性，以启用 WebView 的条件访问。  
...
*   [使用的其他BrowserArguments][ReferenceWin32Icorewebview2environmentoptionsPutAdditionalbrowserarguments09622] `ICoreWebView2EnvironmentOptions` 集不会替代环境变量或注册表值。  有关详细信息，请导航到 [CreateCoreWebView2EnvironmentWithOptions][ReferenceWin32IdlCreatecorewebview2environmentwithoptions09622]。  

## 0.9.579  

发布日期：2020 年 7 月 20 日  

[NuGet 包][NuGetGallery0.9.579] \|最低 Microsoft Edge 版本 86.0.579.0。  

#### 常规  

*   > [!IMPORTANT]
    > **公告**：已发布用于预览的常青 WebView2 运行时和安装程序。  有关详细信息，请导航到 [分发 WebView2][Webview2ConceptsDistributionUnderstandRuntimeInstallerPreview]。  
*   > [!IMPORTANT]
...
*   在 WebView 中禁用弹出窗口阻止程序。  有关详细信息，请导航到 [事件中的 IsUserInitiated][ReferenceWin32Icorewebview2newwindowrequestedeventargsGetIsuserinitiated09538] `NewWindowRequested` 属性。  
*   确保为触发的 WebView 导航启动事件 `about:blank` 。  现在，将针对所有导航触发事件，但不支持和忽略 `NavigationStarting` `about:blank` 取消或 iframe srcdoc。  
*   `edge:// URI`WebView 中的阻止方案。  
*   在 WebView2 环境选项上添加了实验性 [IsSingleSignOnUsingOSPrimaryAccountEnabled][ReferenceWin32Icorewebview2experimentaloptionsGetIssinglesignonusingosprimaryaccountenabled09538] 属性，以启用 WebView 的条件访问。  
*   添加了实验 [性 WebResourceResponseReceived][ReferenceWin32Icorewebview2experimentalAddWebresourceresponsereceived09538] 事件，该事件在 WebView 收到并处理 WebResource 请求的响应后触发。  响应对象中包含身份验证标头（如果有）。  
...
[NuGet 包][NuGetGallery0.9.538] \|最低 Microsoft Edge 版本 85.0.538.0。  
...
*   更新 [了 AddHostObjectToScript][ReferenceWin32Icorewebview2Addhostobjecttoscript09538] 以确保将原始主机对象序列化器标记设置为代理对象，当在 JavaScript 回调 \ ([#148][GithubMicrosoftedgeWebviewfeedbackIssue148]\) 中作为参数传递时，序列化回主机对象。  
...
[NuGet 包][NuGetGallery0.9.515-prerelease] \|最低 Microsoft Edge 版本 84.0.515.0。  

*   > [!IMPORTANT]
    > **公告**：WebView2 现在支持预发行包中的 .NET Framework 4.6.2 或更高版本和 .NET Core 3.0 或更高版本上的 Windows Forms **和**WPF。  
*   有关生成 WPF 应用程序的信息，请导航到 [WPF][GettingstartedWpf] 入门指南和适用于 WPF 特定 API 的 WebView2 [WPF][DotnetApiMicrosoftWebWebview2Wpf] 参考。  
*   有关生成 Windows 窗体应用程序的信息，请导航到 [Windows 窗体][GettingstartedWinforms] 入门指南和适用于 Windows 窗体特定 API 的 WebView2 [Windows][DotnetApiMicrosoftWebWebview2Winforms] 窗体参考。  
...
[NuGet 包][NuGetGallery0.9.488] \|最低 Microsoft Edge 版本 84.0.488.0。  

*   > [!IMPORTANT]
    > **公告**：从即将推出的 Microsoft Edge 版本 83 开始，常青 WebView 不再面向稳定浏览器通道。  相反，它面向另一组二进制文件，品牌为 [Evergreen WebView2 Runtime，][ConceptsDistributionEvergreenMode]可以通过 WebView 团队当前正在开发的安装程序进行链接安装。  有关详细信息，请导航到["应用分发"。][ConceptsDistribution]  
*   > [!IMPORTANT]
...
*   添加了 [FrameNavigationCompleted][ReferenceWin32Icorewebview2AddFramenavigationcompleted09488] 事件。  现在，当 iframe 完成导航时，将触发事件并返回导航和导航 ID 的成功。  
*   添加了 [ICoreWebView2EnvironmentOptions][ReferenceWin32Icorewebview2environmentoptions09488] 接口，可用于确定应用程序目标 WebView2 运行时的版本。  
*   添加了 [IsBuiltInErrorPageEnabled][ReferenceWin32Icorewebview2settingsGetIsbuiltinerrorpageenabled09488] 设置。  现在，您可以选择针对导航失败和呈现进程失败启用或禁用内置错误页。  
...
[NuGet 包][NuGetGallery0.9.430] \|最低 Microsoft Edge 版本 82.0.430.0。  

WebView2 SDK 是官方 Win32 C++ Beta 版本，它包含来自反馈的多项功能请求。  WebView 团队尝试通过发生重大变化来限制发布数量。  随着我们接近正式发布，Beta 版本中将包含一些重大重大更改。  
...
*   > [!IMPORTANT]
    > **重大更改**：将[DocumentStateChanged][ReferenceWin32Iwebview2webviewAddDocumentstatechanged08190]拆分为三个组件[：SourceChanged、ContentLoading][ReferenceWin32Icorewebview2AddSourcechanged09430]和[HistoryChanged。][ReferenceWin32Icorewebview2AddHistorychanged09430] [][ReferenceWin32Icorewebview2AddContentloading09430]  现在，当源 URL 更改时 `SourceChanged` ，将触发该事件。  当历史记录状态更改时 `HistoryChanged` ，将触发事件。  `ContentLoading`加载新文档时，该事件在初始脚本之前触发。  
*   添加了对 ARM64 体系结构的支持。  
*   添加了对触摸屏设备的 (SIP\) 支持。  
*   增加了对 Windows Server 2008 R2、Windows Server 2012、Windows Server 2012 R2 和 Windows Server 2016 的支持。  
*   添加了 [NotifyParentWindowPositionChanged，][ReferenceWin32Icorewebview2hostNotifyparentwindowpositionchanged09430]允许状态栏在窗口模式下跟随窗口。  此外，在无窗口模式下实现此更改，以便辅助功能正常工作。  
*   添加了 [AreRemoteObjectsAllowed][ReferenceWin32Icorewebview2settingsGetAreremoteobjectsallowed09430] 设置，以全局控制任何远程对象是否可以访问页面。  默认情况下启用，这意味着 `AreRemoteObjectsAllowed` [AddRemoteObject][ReferenceWin32Icorewebview2Addremoteobject09430] 添加的远程对象可从页面访问。  禁用 `AreRemoteObjectsAllowed` 后，无法从页面访问对象。  更改将应用于下一个导航事件。  
*   添加了[IsZoomControlEnabled][ReferenceWin32Icorewebview2settingsGetIszoomcontrolenabled09430]设置，以防止用户使用 \ (或 + 鼠标滚轮 `ctrl` + `+` `ctrl` + `-` `ctrl` \) 。  禁用该设置时，仍可 [put_ZoomFactor][ReferenceWin32Icorewebview2hostPutZoomfactor09430] 设置缩放。  
*   将 ZoomFactor 更改为仅应用于当前 WebView。  对当前 WebView 的缩放更改不会影响已导航到同一源网站的其他 WebView。  有关详细信息，请 [导航到][ReferenceWin32Icorewebview2hostGetZoomfactor09430]get_ZoomFactor。  
...
[NuGet 包][NuGetGallery0.8.355] \|最低 Microsoft Edge 版本 80.0.355.0。

*   发布的 WebView2API 示例是 WebView2 SDK 的综合指南。  有关详细信息，请导航到 [APISample][GithubMicrosoftedgeWebview2samplesApisample]。  
...
[NuGet 包][NuGetGallery0.8.314] \|最低 Microsoft Edge 版本 80.0.314.0。  

*   添加了对 Windows 7、Windows 8 和 Windows 8.1 的支持。  
*   添加了Visual Studio和Visual Studio WebView2 的代码调试支持。  现在，从 IDE 调试 WebView2 中的脚本。  有关详细信息，请导航到 [使用 WebView2 控件进行开发时如何调试][HowtoDebug]。  
*   已 `Native Object Injection` 添加，允许运行在 WebView2 中的脚本从应用程序的 Win32 组件访问 IDispatch 对象，并访问 IDispatch 对象的属性。  有关详细信息，请导航到 [AddRemoteObject][ReferenceWin32Iwebview2webview4Addremoteobject08190] \ ([#17][GithubMicrosoftedgeWebviewfeedbackIssue17]\) 。  
...
*   已 `DPI Awareness` 更新。  现在，WebView 的 DPI 感知与主机应用程序的 DPI 感知相同。  
    
    > [!NOTE]
    > 如果使用与原始 WebView 不同的 DPI 感知启动另一个混合应用程序，则如果用户数据目录与 \ (#1 \) 相同，则不启动 [新的][GithubMicrosoftedgeWebviewfeedbackIssue1]WebView。  
    
*   进行了更新，以便 WebView2 自动拒绝 WebView 内承载的 Web 内容所提示 `Notification Change Behavior` 的通知权限请求。  

## 0.8.270  

[NuGet 包][NuGetGallery0.8.270] \|最低 Microsoft Edge 版本 78.0.270.0。  
...
*   更新 `CreateWebView2EnvironmentWithDetails` 了要删除的函数 `releaseChannelPreference` 。  有关函数详细信息， `CreateWebView2EnvironmentWithDetails` 请导航到 [CreateWebView2EnvironmentWithDetails][ReferenceWin32WebView2IdlCreatewebview2environmentwithdetails08190]。  注册表和环境变量替代仍受支持。  除非重写，否则使用默认通道首选项。  
    ...
    WebView 团队选择更稳定的频道，以确保最终用户的行为最一致。  使用最新的 Canary 版本进行测试时，应在启动应用之前创建一个脚本，将环境变量 `WEBVIEW2_RELEASE_CHANNEL_PREFERENCE` `1` 设置为该脚本。  
*   使用 `CreateWebView2EnvironmentWithDetails` 用于选择未指定时 `userDataFolder` 的逻辑更新函数。  有关函数详细信息， `CreateWebView2EnvironmentWithDetails` 请导航到[CreateWebView2EnvironmentWithDetails。][ReferenceWin32WebView2IdlCreatewebview2environmentwithdetails08190]  如果您之前使用默认位置，则切换到新 SDK 时，默认设置为 \ (设置为主机代码目录中 `userDataFolder` 的新位置\) 并且您的状态也会重置 `userDataFolder` 。  
    如果主机进程无权写入指定目录， `CreateWebView2EnvironmentWithDetails` 则函数可能会失败。  你可以将数据从旧用户数据目录复制到新目录。  

## 0.8.230  

[NuGet 包][NuGetGallery0.8.230] \|最低 Microsoft Edge 版本 77.0.230.0。  
...
[NuGet 包][NuGetGallery0.8.190] \|最低 Microsoft Edge 版本 77.0.190.0。  
...
*   在脚本中 `window.open()` 启用请求。  

## 0.8.149  

[NuGet 包][NuGetGallery0.8.149] \|最低 Microsoft Edge 版本 76.0.149.0。  
